...
describe EsxCloud::GoCliClient do
  ...
  it "creates a resource ticket" do
    ...
    expect(client).to receive(:run_cli).with("resource-ticket create -t '#{tenant_name}' -n 'rt' -l 'a b c, d e f'")
    expect(client).to receive(:find_resource_ticket_by_name).and_return(ticket)
  ...
  it "finds all tickets" do
    ...
    expect(client).to receive(:get_resource_ticket_list_from_response).with(result,tenant_id).and_return(tickets)
