  ...
  class GoCliClient
    module DiskApi
      ...
      def get_disk_from_response(result)
        ...
        unless values[1] == ""
          disk_hash["name"] = values[1]
        end
        unless values[2] == ""
          disk_hash["state"] = values[2]
        end
        unless values[3] == ""
          disk_hash["kind"] = values[3]
        end
        unless values[4] == ""
          disk_hash["flavor"] = values[4]
        end
        unless values[5] == ""
          disk_hash["capacityGb"] = values[5].to_i
        end
        unless values[6] == ""
          disk_hash["datastore"] = values[6]
        end
