  ...
  class GoCliClient
    module HostApi
      ...
      def get_host_from_response(result)
        ...
        host_hash = Hash.new
        unless values[0] == ""
          host_hash["id"] = values[0]
        end
        unless values[1] == ""
          host_hash["username"] = values[1]
        end
        unless values[2] == ""
          host_hash["password"] = values[2]
        end
        unless values[3] == ""
          host_hash["address"] = values[3]
        end
        unless values[4] == ""
          host_hash["usageTags"] = values[4]
        end
        unless values[5] == ""
          host_hash["state"] = values[5]
        end
        host_hash["metadata"] = metadata_to_hash(values[6])
        unless values[7] == ""
          host_hash["availabilityZone"] = values[7]
        end
        unless values[8] == ""
          host_hash["esxVersion"] = values[8]
        end
