  ...
  class GoCliClient
    module ResourceTicketApi
      ...
      def create_resource_ticket(tenant_id, payload)
        ...
        find_resource_ticket_by_name(tenant_id, payload[:name])
      end
      ...
      def find_all_resource_tickets(tenant_id)
        ...
        result = run_cli(cmd)
        get_resource_ticket_list_from_response(result, tenant_id)
      end
      ...
      # @param [String] tenant_id
      # @return [ResourceTicketList]
      def get_resource_ticket_list_from_response(result, tenant_id)
        resource_tickets = result.split("\n").drop(1).map do |resource_ticket|
          find_resource_ticket_by_name(tenant_id,resource_ticket.split[1])
        end
