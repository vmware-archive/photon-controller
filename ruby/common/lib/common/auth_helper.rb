...
module EsxCloud
  class AuthHelper
    ...
    def self.get_access_token(username, password, service_locator_url, auth_tool_path = nil)
      ...
      password_grant_format_string =
          "grant_type=password&username=#{username}&password=#{password}&scope=openid offline_access rs_esxcloud at_groups"

      http_client = HttpClient.new(service_locator_url)
      access_token = http_client.post(token_path, password_grant_format_string,
