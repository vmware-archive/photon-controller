...
describe "virtual_network", :virtual_network => true do
  ...
  let(:spec) { EsxCloud::VirtualNetworkCreateSpec.new(virtual_network_name, "virtual network", "ROUTED", 128, 16) }
  ...
  describe "#create" do
    it "should create one ROUTED virtual network successfully" do
      ...
      networks = client.find_virtual_networks_by_name(@project.id, virtual_network_name).items
    ...
    it "should create two virtual network successfully with the same names" do
      ...
      networks = client.find_virtual_networks_by_name(@project.id, spec.name).items
  ...
  def create_virtual_network(project_id, spec)
    begin
      ...
      EsxCloud::VirtualNetwork.find_by_name(project_id, spec.name).items.each {|i| virtual_networks_to_delete << i}
      raise
    end
  end
