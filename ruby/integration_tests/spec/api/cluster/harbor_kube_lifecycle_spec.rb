...
describe "Harbor-Kube cluster-service lifecycle", cluster: true do
  ...
  it 'should create/delete Harbor cluster successfully' do
    ...
    begin
      ...
      harbor_cluster = create_harbor_cluster(project, harbor_props)
      validate_harbor_cluster_info(harbor_cluster.id)
      export_ca_cert(harbor_cluster.id)
      validate_ssh(harbor_master_ip)

      puts "Create a kubernetes cluster"
      kube_props = construct_kube_properties(kubernetes_master_ip, kubernetes_etcd_ip)
      ...
      validate_kube_api_responding(kubernetes_master_ip, 2)
      validate_ssh(kubernetes_master_ip)
      ...
      delete_harbor_cluster(harbor_cluster.id)
      delete_kube_cluster(kubernetes_cluster.id)
  ...
  def delete_harbor_cluster(cluster_id)
  ...
  def delete_kube_cluster(cluster_id)
  ...
  def validate_kube_cluster_info(cluster_current, expected_worker_count, flavor)
    puts "Validating cluster info before resizing ..."
    ...
    puts "Validated cluster info before resizing ..."
  end
  ...
  def construct_kube_properties(master_ip, etcd_ip)
    public_key_contents = File.read("/tmp/test_rsa.pub")
  ...
  def create_kubernetes_application_workflow()
    ...
    copying_over_the_kubectl_from_master_node(ssh)
    creating_kubernetes_application(ssh)
  end

  end

  def copy_ca_cert_to_devbox(ssh)
    puts "Creating ca_cert forlder in devboxto be used by docker"
  ...
  def copying_over_the_kubectl_from_master_node(ssh)

    puts "copying the kubectl from master node"
  ...
  def creating_kubernetes_application(ssh)
    ...
    populating_busybox_config_file()
    ...
    puts "Sleeping for 5 mins to let the pod come up."
    sleep(300)
  ...
  def populating_busybox_config_file()
