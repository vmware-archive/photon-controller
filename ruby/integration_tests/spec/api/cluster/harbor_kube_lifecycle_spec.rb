...
describe "Harbor-Kube cluster-service lifecycle", cluster: true do
  ...
  it 'should create/delete Harbor cluster successfully' do
    ...
    begin
      ...
      copy_harbor_ca_cert_cmd = "sshpass -p \"vmware\" scp /tmp/harbor_ca_cert.crt esxcloud@#{ENV['API_ADDRESS']}:/tmp/"
      ...
      copy_test_rsa_cmd = "sshpass -p \"vmware\" scp /tmp/test_rsa esxcloud@#{ENV['API_ADDRESS']}:/tmp/"
  ...
  def copy_ca_cert_to_devbox(ssh)
    ...
    # Logging for testing. Will be removed later
    ls_cmd = "sudo ls -lh /tmp"
    puts copy_cmd
    output = ssh.exec!(copy_cmd)
    puts output
  end
  ...
  def populate_busybox_config_file()
    ...
    copy_busybox_yaml_cmd = "sshpass -p \"vmware\" scp /tmp/busybox_updated.yaml esxcloud@#{ENV['API_ADDRESS']}:/tmp/"
