...
describe "vm", management: true, image: true do
  ...
  it "should create a vm with multiple nics on different networks", disable_for_cli_test: true, single_vm_port_group: true  do
    begin
      ...
      expect(network_ids).to contain_exactly(network1.id, network2.id)
    ensure
  ...
  it "should create a vm with multiple nics on same network", disable_for_cli_test: true do
    begin
      network1 = EsxCloud::SystemSeeder.instance.network!

      vm_name = random_name("vm-")
      create_vm(@project, name: vm_name, networks: [network1.id, network1.id])
      ...
      networks = client.get_vm_networks(vms[0].id).network_connections
      network_ids = networks.map { |n| n.id }
      expect(network_ids).to contain_exactly(network1.id, network1.id)
    ensure
      ignoring_all_errors { vm.delete if vm }
      ignoring_all_errors { network2.delete if network2 }
    end
