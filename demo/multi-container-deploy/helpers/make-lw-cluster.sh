...
docker create \
  ...
  --name photon-config-data \
   vmware/photon-controller-seed:1.0.3 /bin/true
...
./helpers/run-lw-container.sh $LIGHTWAVE_PARTNER_0 $LIGHTWAVE_PARTNER_0 $LIGHTWAVE_PASSWORD standalone 1

if [ "$MULTI_HOST" == "1" ]; then
  ./helpers/run-lw-container.sh $LIGHTWAVE_PARTNER_1 $LIGHTWAVE_PARTNER_0 $LIGHTWAVE_PASSWORD partner 2
  ./helpers/run-lw-container.sh $LIGHTWAVE_PARTNER_2 $LIGHTWAVE_PARTNER_0 $LIGHTWAVE_PASSWORD partner 3
fi
