...
(set -x;
docker run -d --volumes-from photon-config-${NUMBER} --name volume-helper vmware/photon-controller-seed:$PC_CONTAINER_VERSION /bin/sh -c "while true; do ping 8.8.8.8; done")
