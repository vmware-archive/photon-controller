...
./helpers/make-pc-cluster.sh $MULTI_CONTAINER_PC "$LIGHTWAVE_PASSWORD" $LIGHTWAVE_DOMAIN $ENABLE_FQDN $PC_CONTAINER_VERSION
if [ $? -ne 0 ]; then
...
./helpers/make-ui-cluster.sh $UI_CONTAINER_VERSION
UI_AVAILABLE=1
if [ $? -ne 0 ]; then
