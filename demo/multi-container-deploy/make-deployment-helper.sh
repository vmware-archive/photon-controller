...
while [ $attempts -lt $total_attempts ] && [ $certs_created != "true" ]; do
   if [ ! -f /etc/keys/machine.crt -o ! -f /etc/keys/machine.privkey ]; then
...
while [ $attempts -lt $total_attempts ] && [ $pc_running != "true" ]; do
   nodes=$(curl --key /etc/keys/machine.privkey --cert /etc/keys/machine.crt --capath /etc/ssl/certs https://$PHOTON_CONTROLLER_HOST_IP:19000/core/node-groups/default | grep AVAILABLE | wc -l)
   if [ "$nodes" != "3" ]; then
