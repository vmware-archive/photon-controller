...
public class ValidateHostTaskService extends StatefulService {
  ...
  private void validateMgmtVmIpNotAssigned(State state, Operation allMgmtHostsQuery, Map<Long, Operation> os)
    ...
    // since we add the host document to cloudstore first and then validate the document there should be
    // exactly one document that is using this management ip
    if (mgmtHostsWithSameMgmtVmIp.size() > 1) {
  ...
  private Operation buildAllMgmtHostsQuery(URI cloudStoreUri) {
    ...
    return Operation
        .createPost(UriUtils.buildBroadcastRequestUri(
            UriUtils.buildUri(cloudStoreUri, ServiceUriPaths.XENON.CORE_LOCAL_QUERY_TASKS),
