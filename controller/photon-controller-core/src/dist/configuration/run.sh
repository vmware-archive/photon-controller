...
function join_lightwave()
{
  local lw_domain=`get_config_value $PHOTON_CONTROLLER_CORE_CONFIG authDomain:`
  local lw_host_address=`get_config_value $PHOTON_CONTROLLER_CORE_CONFIG authServerAddress:`
  local lw_password=`get_config_value $PHOTON_CONTROLLER_CORE_CONFIG authPassword:`
  local registration_address=`get_config_value $PHOTON_CONTROLLER_CORE_CONFIG registrationAddress:`
  local ic_join_params=""

  if [ -n "$lw_host_address" ]
  then
    ic_join_params="$ic_join_params --domain-controller $lw_host_address"
  fi

  if [ -n "$lw_domain" ]
  then
    ic_join_params="$ic_join_params --domain $lw_domain"
  fi

  if [ -n "$lw_password" ]
  then
    ic_join_params="$ic_join_params --password $lw_password"
  fi
