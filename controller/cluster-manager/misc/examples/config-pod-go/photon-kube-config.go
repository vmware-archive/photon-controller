...
// Find key in the config string and removes it,
// then adds the new key/value from secret map data to the config.
func getKeyAndUpdateConfig(data map[string][]byte, key string, config string) string {
	// Find old key/value in the old config and remove it.
	keyPattern := fmt.Sprintf("(?m)[\r\n]+^.*%s.*$", key)
	re := regexp.MustCompile(keyPattern)
	config = re.ReplaceAllString(config, "")

	// Add new key/value if present in the secret data map.
