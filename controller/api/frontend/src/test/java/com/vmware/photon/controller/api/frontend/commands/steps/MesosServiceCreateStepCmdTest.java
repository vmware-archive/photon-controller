...
/**
 * Tests {@link MesosServiceCreateStepCmd}.
 */
public class MesosServiceCreateStepCmdTest extends PowerMockTestCase {

  MesosServiceCreateStepCmd command;
  ...
  private MesosServiceCreateTaskState serviceDocument;
  ...
  private void setUpCommon() {
    ...
    createSpec.setExtendedProperties(ImmutableMap.<String, String>builder()
        ...
        .put(ServicesManagerClient.EXTENDED_PROPERTY_ZOOKEEPER_IP1, "10.1.0.3")
        .put(ServicesManagerClient.EXTENDED_PROPERTY_ZOOKEEPER_IP2, "10.1.0.4")
        .put(ServicesManagerClient.EXTENDED_PROPERTY_ZOOKEEPER_IP3, "10.1.0.5")
        .build());

    currentStep = new StepEntity();
    currentStep.setId("id");
    currentStep.createOrUpdateTransientResource(MesosServiceCreateStepCmd.PROJECT_ID_RESOURCE_KEY, "projectId");
    currentStep.createOrUpdateTransientResource(MesosServiceCreateStepCmd.CREATE_SPEC_RESOURCE_KEY, createSpec);
    ...
    command = spy(new MesosServiceCreateStepCmd(taskCommand, stepBackend, currentStep, serviceBackend));
    when(serviceBackend.getServicesManagerClient()).thenReturn(servicesManagerClient);

    serviceDocument = new MesosServiceCreateTaskState();
    serviceDocument.taskState = new MesosServiceCreateTaskState.TaskState();
    serviceDocument.taskState.stage = TaskState.TaskStage.STARTED;
    serviceDocument.taskState.subStage = MesosServiceCreateTaskState.TaskState.SubStage.SETUP_ZOOKEEPERS;
    remoteTaskLink = "http://servicesmanager" + ServiceUriPaths.MESOS_SERVICE_CREATE_TASK
  ...
  public class ExecuteTest {
    ...
    public void testSuccess() throws Throwable {
      when(servicesManagerClient.createMesosService(any(String.class), any(ServiceCreateSpec.class)))
          .thenReturn(serviceDocument);

      command.execute();
      verify(servicesManagerClient, times(1)).createMesosService(any(String.class), any(ServiceCreateSpec.class));
    ...
    public void testMissingProjectId() throws Throwable {
      currentStep.createOrUpdateTransientResource(MesosServiceCreateStepCmd.PROJECT_ID_RESOURCE_KEY, null);
      command = new MesosServiceCreateStepCmd(taskCommand, stepBackend, currentStep, serviceBackend);
    ...
    public void testMissingCreateSpec() throws Throwable {
      currentStep.createOrUpdateTransientResource(MesosServiceCreateStepCmd.CREATE_SPEC_RESOURCE_KEY, null);
      command = new MesosServiceCreateStepCmd(taskCommand, stepBackend, currentStep, serviceBackend);
