...
/**
 * Tests {@link com.vmware.photon.controller.api.frontend.auth.fetcher.SubnetSecurityGroupFetcher}.
 */
public class SubnetSecurityGroupFetcherTest {

  private VirtualNetworkFeClient virtualNetworkFeClient;
  private ProjectSecurityGroupFetcher projectFetcher;
  private SubnetSecurityGroupFetcher fetcher;
  ...
  public class FetchSecurityGroupsTest {
    ...
    @BeforeMethod
    private void setUp() {
      virtualNetworkFeClient = mock(VirtualNetworkFeClient.class);
      projectFetcher = mock(ProjectSecurityGroupFetcher.class);
      fetcher = new SubnetSecurityGroupFetcher(virtualNetworkFeClient, projectFetcher);
    ...
    @Test(expectedExceptions = IllegalArgumentException.class,
        expectedExceptionsMessageRegExp = "authorizationObject must be of 'kind' SUBNET.")
    ...
    public void testInvalidId() throws Throwable {
      doThrow(new NetworkNotFoundException("id")).when(virtualNetworkFeClient).get("id");
    ...
    public void testParentWithoutSecurityGroups() throws Throwable {
      ...
      subnet.setParentId("parent-id");
      doReturn(subnet).when(virtualNetworkFeClient).get("id");
      ...
      Set<String> groups = fetcher.fetchSecurityGroups(authorizationObject);
      assertThat(groups.size(), is(0));
      assertThat(captor.getValue().getId(), is("parent-id"));
    }
