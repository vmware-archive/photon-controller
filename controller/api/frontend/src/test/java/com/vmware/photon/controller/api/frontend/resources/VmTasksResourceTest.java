...
/**
 * Tests {@link com.vmware.photon.controller.api.frontend.resources.tasks.VmTasksResource}.
 */
public class VmTasksResourceTest extends ResourceTest {
  ...
  protected void setUpResources() throws Exception {
    ...
    addResource(new VmTasksResource(client, paginationConfig));
  }

  @Test(dataProvider = "pageSizes")
  public void testGetVmTasks(Optional<Integer> pageSize,
                             List<Task> expectedTasks,
                             List<String> expectedTaskRoutes) throws Exception {
    task1.setId(taskId1);
    task2.setId(taskId2);

    when(client.getVmTasks(vmId, Optional.<String>absent(), Optional.of(PaginationConfig.DEFAULT_DEFAULT_PAGE_SIZE)))
        .thenReturn(new ResourceList<>(ImmutableList.of(task1, task2), null, null));
    when(client.getVmTasks(vmId, Optional.<String>absent(), Optional.of(1)))
        .thenReturn(new ResourceList<>(ImmutableList.of(task1), UUID.randomUUID().toString(), null));
    when(client.getVmTasks(vmId, Optional.<String>absent(), Optional.of(2)))
        .thenReturn(new ResourceList<>(ImmutableList.of(task1, task2), null, null));
    when(client.getVmTasks(vmId, Optional.<String>absent(), Optional.of(3)))
  ...
  @Test
  public void testGetVmTasksPage() throws Exception {
    task1.setId(taskId1);
    task2.setId(taskId2);
  ...
  private Response getTasks(String pageLink) {
    String uri = vmTaskRoute + "?pageLink=" + pageLink;
