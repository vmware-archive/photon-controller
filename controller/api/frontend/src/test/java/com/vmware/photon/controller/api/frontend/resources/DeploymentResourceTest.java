...
/**
 * Tests {@link com.vmware.photon.controller.api.frontend.resources.deployment.DeploymentResource}.
 */
public class DeploymentResourceTest extends ResourceTest {
  ...
  @Test
  public void testConfigureNsx() throws Exception {
    Task task = new Task();
    task.setId(taskId);
    doReturn(task).when(feClient).configureNsx(eq(deploymentId), any(NsxConfigurationSpec.class));

    String uri = UriBuilder
        .fromPath(DeploymentResourceRoutes.DEPLOYMENT_PATH + DeploymentResourceRoutes.CONFIGURE_NSX_ACTION)
        .build(deploymentId)
        .toString();
