...
/**
 * Implementation of object that retrieves the security groups for a service authorization
 * object.
 */
public class ServiceSecurityGroupFetcher implements SecurityGroupFetcher {
  /**
   * Logger.
   */
  private static final Logger logger = LoggerFactory.getLogger(ServiceSecurityGroupFetcher.class);

  /**
   * Storage access object for service resources.
   */
  ...
  @Inject
  public ServiceSecurityGroupFetcher(ServiceFeClient serviceFeClient,
  ...
  public Set<String> fetchSecurityGroups(TransactionAuthorizationObject authorizationObject) {
    checkArgument(authorizationObject.getKind() == TransactionAuthorizationObject.Kind.SERVICE,
        "authorizationObject must be of 'kind' SERVICE.");
    ...
    try {
      ...
      logger.warn("invalid service id {}", authorizationObject.getId());
    } catch (Exception ex) {
      logger.warn("unexpected exception processing service {}", authorizationObject.getId());
    }
