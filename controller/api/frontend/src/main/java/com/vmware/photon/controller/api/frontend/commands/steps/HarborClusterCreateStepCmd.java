...
/**
 * StepCommand that kicks off Harbor cluster creation.
 */
public class HarborClusterCreateStepCmd extends StepCommand {
  private static final Logger logger = LoggerFactory.getLogger(HarborClusterCreateStepCmd.class);
  ...
  public HarborClusterCreateStepCmd(TaskCommand taskCommand, StepBackend stepBackend,
  ...
  protected void execute() throws SpecInvalidException {
    ...
    logger.info("HarborClusterCreateStepCmd started, projectId={}, clusterName={}", projectId, spec.getName());

    HarborClusterCreateTask serviceDocument = clusterBackend.getClusterManagerClient()
        .createHarborCluster(projectId, spec);
    ...
    logger.info("Harbor cluster creation initiated: id={}", serviceDocument.documentSelfLink);
  }
