...
/**
 * Resource APIs for Vms in a service.
 */
@Path(ServiceResourceRoutes.SERVICES_VMS_PATH)
@Api(value = ServiceResourceRoutes.API)
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class ServiceVmsResource {

  private final ServiceFeClient serviceFeClient;
  private final PaginationConfig paginationConfig;

  @Inject
  public ServiceVmsResource(ServiceFeClient serviceFeClient, PaginationConfig paginationConfig) {
    this.serviceFeClient = serviceFeClient;
    this.paginationConfig = paginationConfig;
  }

  @GET
  @ApiOperation(value = "List VMs in a service", response = Vm.class, responseContainer = ResourceList.CLASS_NAME)
  @ApiResponses(value = {@ApiResponse(code = 200, message = "List of VMs in the service")})
  public Response list(@Context Request request,
    ...
    if (pageLink.isPresent()) {
      ...
      resourceList = serviceFeClient.findVms(serviceId, adjustedPageSize);
    }

    String apiRoute = UriBuilder.fromPath(ServiceResourceRoutes.SERVICES_VMS_PATH).build(serviceId).toString();
