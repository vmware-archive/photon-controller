...
/**
 * StepCommand that kicks off Harbor service creation.
 */
public class HarborServiceCreateStepCmd extends StepCommand {
  private static final Logger logger = LoggerFactory.getLogger(HarborServiceCreateStepCmd.class);
  ...
  private final ServiceCreateSpec spec;

  public HarborServiceCreateStepCmd(TaskCommand taskCommand, StepBackend stepBackend,
    ...
    spec = (ServiceCreateSpec) step.getTransientResource(CREATE_SPEC_RESOURCE_KEY);
  }

  @Override
  protected void execute() throws SpecInvalidException {
    checkArgument(projectId != null, "project-id is not defined in TransientResource");
    checkArgument(spec != null, "create-spec is not defined in TransientResource");

    logger.info("HarborServiceCreateStepCmd started, projectId={}, serviceName={}", projectId, spec.getName());

    HarborServiceCreateTaskState serviceDocument = serviceBackend.getServicesManagerClient()
        .createHarborService(projectId, spec);
    // pass remote task link to XenonTaskStatusStepCmd
    ...
    logger.info("Harbor service creation initiated: id={}", serviceDocument.documentSelfLink);
  }
