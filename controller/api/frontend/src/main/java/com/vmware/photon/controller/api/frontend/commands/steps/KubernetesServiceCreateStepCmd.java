...
/**
 * StepCommand that kicks off Kubernetes service creation.
 */
public class KubernetesServiceCreateStepCmd extends StepCommand {
  private static final Logger logger = LoggerFactory.getLogger(KubernetesServiceCreateStepCmd.class);
  ...
  public KubernetesServiceCreateStepCmd(TaskCommand taskCommand, StepBackend stepBackend,
  ...
  protected void execute() throws SpecInvalidException {
    ...
    logger.info("KubernetesServiceCreateStepCmd started, projectId={}, serviceName={}", projectId, spec.getName());

    KubernetesServiceCreateTaskState serviceDocument = serviceBackend.getServicesManagerClient()
        .createKubernetesService(projectId, spec);
