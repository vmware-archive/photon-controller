...
import com.vmware.photon.controller.servicesmanager.servicedocuments.HarborServiceCreateTaskState.TaskState.SubStage;
...
/**
 * Polls Harbor service creation task status.
 */
public class HarborServiceCreateTaskStatusPoller implements XenonTaskStatusStepCmd.XenonTaskStatusPoller {
  private static final Map<Operation, Integer> OPERATION_TO_SUBSTAGE_MAP =
      ImmutableMap.<Operation, Integer>builder()
          .put(Operation.CREATE_HARBOR_SERVICE_SETUP_HARBOR, SubStage.SETUP_HARBOR.ordinal())
          .put(Operation.CREATE_HARBOR_SERVICE_UPDATE_EXTENDED_PROPERTIES,
  ...
  public HarborServiceCreateTaskStatusPoller(TaskCommand taskCommand, ServiceBackend serviceBackend,
  ...
  public TaskState poll(String remoteTaskLink) throws DocumentNotFoundException, TaskNotFoundException {
    HarborServiceCreateTaskState serviceDocument = serviceBackend.getServicesManagerClient()
        .getHarborServiceCreationStatus(remoteTaskLink);
  ...
  public int getSubStage(TaskState taskState) {
    return ((HarborServiceCreateTaskState.TaskState) taskState).subStage.ordinal();
  }
