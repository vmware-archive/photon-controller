...
import com.vmware.photon.controller.servicesmanager.servicedocuments.ServiceResizeTaskState.TaskState.SubStage;
...
/**
 * Polls service resize task status.
 */
public class ServiceResizeTaskStatusPoller implements XenonTaskStatusStepCmd.XenonTaskStatusPoller {
  private static final Map<Operation, Integer> OPERATION_TO_SUBSTAGE_MAP =
      ImmutableMap.<Operation, Integer>builder()
          .put(Operation.RESIZE_SERVICE_INITIALIZE_SERVICE, SubStage.INITIALIZE_SERVICE.ordinal())
          .put(Operation.RESIZE_SERVICE_RESIZE, SubStage.RESIZE_SERVICE.ordinal())
          .build();

  private final ServiceBackend serviceBackend;

  public ServiceResizeTaskStatusPoller(ServiceBackend serviceBackend) {
  ...
  public TaskState poll(String remoteTaskLink) throws DocumentNotFoundException {
    ServiceResizeTaskState serviceDocument = serviceBackend.getServicesManagerClient()
        .getServiceResizeStatus(remoteTaskLink);
  ...
  public int getSubStage(TaskState taskState) {
    return ((ServiceResizeTaskState.TaskState) taskState).subStage.ordinal();
  }
