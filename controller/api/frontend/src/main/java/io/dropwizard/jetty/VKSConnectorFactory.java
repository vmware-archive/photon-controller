 ...
 * uses VMware Key Store (VKS) instead of Java Key Store (JKS) which is the default Key Store supported by Dropwizard.
 * We use VKS so that we can use lightwave and manage certificates better in our system since VKS supports fail over
 * and load balancing of lightwave instances. It also allows us to refresh certificates easily. We can also remove
 * the JKS keystore file along with its password which is harder to maintain and secure.
 * This connector factory gets called by supplying "vks" as the application connector type in apife config in the
 * photon controller yml file.
 *
