...
/**
 * This class implements tests for the {@link ServiceResizeTask} class.
 */
public class ServiceResizeTaskTest {

  private TestHost host;
  private ServiceResizeTask taskService;
  ...
  private ServiceResizeTaskState buildValidStartState(
      TaskState.TaskStage stage, ServiceResizeTaskState.TaskState.SubStage subStage) throws Throwable {

    ServiceResizeTaskState state = ReflectionUtils.buildValidStartState(ServiceResizeTaskState.class);
  ...
  private ServiceResizeTaskState buildValidPatchState(
      TaskState.TaskStage stage, ServiceResizeTaskState.TaskState.SubStage subStage) {

    ServiceResizeTaskState patchState = new ServiceResizeTaskState();
    patchState.taskState = new ServiceResizeTaskState.TaskState();
  ...
  public class InitializationTest {
    ...
    public void setUp() throws Throwable {
      taskService = new ServiceResizeTask();
    }
  ...
  public class HandleStartTest {
    ...
    public void testValidStartState(TaskState.TaskStage stage,
                                    ServiceResizeTaskState.TaskState.SubStage subStage) throws Throwable {

      ServiceResizeTaskState startState = buildValidStartState(stage, subStage);

      Operation startOp = host.startServiceSynchronously(taskService, startState);
      assertThat(startOp.getStatusCode(), is(200));
      ServiceResizeTaskState savedState = host.getServiceState(
          ServiceResizeTaskState.class);
    ...
    public Object[][] getValidStartStates() {
      ...
      return new Object[][]{
          ...
          {TaskState.TaskStage.STARTED,
              ServiceResizeTaskState.TaskState.SubStage.INITIALIZE_SERVICE},
          {TaskState.TaskStage.STARTED,
              ServiceResizeTaskState.TaskState.SubStage.RESIZE_SERVICE},
    ...
    public void testMissingStateValue(String attributeName) throws Throwable {
      ServiceResizeTaskState startState = buildValidStartState(null, null);
    ...
    public Object[][] getAttributeNames() {
      List<String> notNullAttributes = ReflectionUtils.getAttributeNamesWithAnnotation(
          ServiceResizeTaskState.class, NotNull.class);
  ...
  public class HandlePatchTest {
    ...
    public void testValidStageUpdates(TaskState.TaskStage startStage,
                                      ServiceResizeTaskState.TaskState.SubStage startSubStage,
                                      TaskState.TaskStage patchStage,
                                      ServiceResizeTaskState.TaskState.SubStage patchSubStage) throws Throwable {

      ServiceResizeTaskState startState = buildValidStartState(startStage, startSubStage);
      host.startServiceSynchronously(taskService, startState);

      ServiceResizeTaskState patchState = buildValidPatchState(patchStage, patchSubStage);
      ...
      ServiceResizeTaskState savedState = host.getServiceState(ServiceResizeTaskState.class);
    ...
    public Object[][] getValidStageUpdates()
      ...
      return new Object[][]{
          {TaskState.TaskStage.CREATED, null,
              TaskState.TaskStage.STARTED, ServiceResizeTaskState.TaskState.SubStage.INITIALIZE_SERVICE},
          {TaskState.TaskStage.CREATED, null,
              TaskState.TaskStage.FAILED, null},
          {TaskState.TaskStage.CREATED, null,
              TaskState.TaskStage.CANCELLED, null},
    ...
    public void testInvalidSubStageUpdates(TaskState.TaskStage startStage,
                                           ServiceResizeTaskState.TaskState.SubStage startSubStage,
                                           TaskState.TaskStage patchStage,
                                           ServiceResizeTaskState.TaskState.SubStage patchSubStage)
        throws Throwable {

      ServiceResizeTaskState startState = buildValidStartState(startStage, startSubStage);
      host.startServiceSynchronously(taskService, startState,
          ServiceMaintenanceTaskFactory.SELF_LINK + TestHost.SERVICE_URI);

      ServiceResizeTaskState patchState = buildValidPatchState(patchStage, patchSubStage);
      Operation patchOp = Operation
          .createPatch(UriUtils.buildUri(host,
              ServiceMaintenanceTaskFactory.SELF_LINK + TestHost.SERVICE_URI, null))
          .setBody(patchState);
    ...
    public void testInvalidPatchImmutableFieldChanged(String fieldName) throws Throwable {
      ServiceResizeTaskState startState = buildValidStartState(TaskState.TaskStage.CREATED, null);
      Operation startOperation = host.startServiceSynchronously(taskService, startState);
      assertThat(startOperation.getStatusCode(), is(200));

      ServiceResizeTaskState patchState = buildValidPatchState(TaskState.TaskStage.STARTED,
          ServiceResizeTaskState.TaskState.SubStage.INITIALIZE_SERVICE);
    ...
    public Object[][] getImmutableFieldNames() {
      return TestHelper.toDataProvidersList(
          ImmutableList.of("controlFlags", "serviceId", "newWorkerCount"));
    }
  }
  ...
  public class EndToEndTest {
    ...
    private ServiceResizeTaskState startState;
    ...
    public void testEndToEndSuccess(Integer currentWorkerCount,
                                    ...
                                    com.vmware.photon.controller.api.model.ServiceState expectedServiceState)
        throws Throwable {

      mockService(currentWorkerCount);
      mockGetServiceVms(5, true);
      mockResizeService(true);
      startState.newWorkerCount = newWorkerCount;

      ServiceResizeTaskState savedState = machine.callServiceAndWaitForState(
          ServiceResizeTaskFactory.SELF_LINK,
          startState,
          ServiceResizeTaskState.class,
      ...
      // Verify that a Service document has been updated
      ...
      ServiceState.State serviceState = cloudStoreMachine.getServiceState(documentLinks.iterator().next(),
          ServiceState.State.class);

      assertThat(serviceState.documentSelfLink, containsString(savedState.serviceId));
      verifyService(savedState.serviceId, newWorkerCount, expectedServiceState);
    }

    @DataProvider(name = "endToEndSuccessData")
    public Object[][] getEndToEndSuccessData() {
      return new Object[][]{
          {5, 5, com.vmware.photon.controller.api.model.ServiceState.READY}, // Delta = 0
          {5, 7, com.vmware.photon.controller.api.model.ServiceState.RESIZING}, // Delta > 0
      };
    }

    @Test
    public void testEndToEndFailureContractServiceFails() throws Throwable {
      ...
      verifyService(savedState.serviceId, 5, com.vmware.photon.controller.api.model.ServiceState.READY);
    }

    private void mockResizeService(boolean isSuccess) throws Throwable {
    ...
    private void mockService(int workerCount) throws Throwable {
      ...
      serviceState.extendedProperties.put(ServicesManagerConstants.EXTENDED_PROPERTY_MASTER_IP, "10.0.0.1");
      ...
      startState.serviceId = ServiceUtils.getIDFromDocumentSelfLink(savedServiceState.documentSelfLink);

      ServiceMaintenanceTask.State maintenanceState = new ServiceMaintenanceTask.State();
      maintenanceState.documentSelfLink = startState.serviceId;
      machine.callServiceAndWaitForState(
          ServiceMaintenanceTaskFactory.SELF_LINK,
          maintenanceState,
    ...
    private void mockGetServiceVms(int nodeCount, boolean isSuccess) throws Throwable {
      if (isSuccess) {
        ...
        vmTags.add(ServicesUtil.createServiceNodeTag(startState.serviceId, NodeType.KubernetesWorker));
    ...
    private void verifyService(
        ...
        com.vmware.photon.controller.api.model.ServiceState expectedServiceState) throws Throwable {
      ...
      assertThat(serviceState.documentSelfLink, containsString(serviceId));
      assertThat(serviceState.workerCount, is(expectedWorkerCount));
      assertThat(serviceState.serviceState, is(expectedServiceState));
    }
  }
}
