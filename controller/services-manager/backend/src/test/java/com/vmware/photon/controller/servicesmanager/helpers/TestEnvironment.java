...
package com.vmware.photon.controller.servicesmanager.helpers;
...
import com.vmware.photon.controller.servicesmanager.ServicesManagerTestServiceGroup;
...
public class TestEnvironment extends MultiHostEnvironment<PhotonControllerXenonHost> {
  ...
  private TestEnvironment(
      int hostCount, ServicesManagerFactory servicesManagerFactory) throws Throwable {
    ...
    for (int i = 0; i < hosts.length; i++) {
      ...
      CloudStoreHelper cloudStoreHelper = servicesManagerFactory.createCloudStoreHelper();
      ...
      ServicesManagerTestServiceGroup servicesManagerTestServiceGroup =
          new ServicesManagerTestServiceGroup(servicesManagerFactory);

      hosts[i].registerDeployer(servicesManagerTestServiceGroup);
  ...
  public static class Builder {
    ...
    public TestEnvironment build() throws Throwable {
      ...
      if (this.etcdClient != null) {
        doReturn(this.etcdClient).when(servicesManagerFactory).createEtcdClient();
      }

      if (this.kubernetesClient != null) {
        doReturn(this.kubernetesClient).when(servicesManagerFactory).createKubernetesClient();
      }

      if (this.mesosClient != null) {
        doReturn(this.mesosClient).when(servicesManagerFactory).createMesosClient();
      }

      if (this.swarmClient != null) {
        doReturn(this.swarmClient).when(servicesManagerFactory).createSwarmClient();
      }

      if (this.harborClient != null) {
        doReturn(this.harborClient).when(servicesManagerFactory).createHarborClient();
      }

      if (this.cloudStoreHelper != null) {
        doReturn(this.cloudStoreHelper).when(servicesManagerFactory).createCloudStoreHelper();
      }

      if (this.apiClient != null) {
        doReturn(this.apiClient).when(servicesManagerFactory).createApiClient(anyObject());
      }

      if (this.scriptsDirectory != null) {
        doReturn(this.scriptsDirectory).when(servicesManagerFactory).getScriptsDirectory();
      }

      if (this.listeningExecutorService != null) {
        doReturn(this.listeningExecutorService).when(servicesManagerFactory).getListeningExecutorServiceInstance();
      }

      if (this.statusCheckHelper != null) {
        doReturn(this.statusCheckHelper).when(servicesManagerFactory).createStatusCheckHelper();
      }

      TestEnvironment environment = new TestEnvironment(hostCount, servicesManagerFactory);
      environment.start();
      return environment;
    }
  }
