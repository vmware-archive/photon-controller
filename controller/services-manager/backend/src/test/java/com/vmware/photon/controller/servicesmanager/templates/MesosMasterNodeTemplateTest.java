...
package com.vmware.photon.controller.servicesmanager.templates;

import com.vmware.photon.controller.servicesmanager.servicedocuments.FileTemplate;
...
/**
 * Implements tests for {@link MesosMasterNodeTemplate}.
 */
public class MesosMasterNodeTemplateTest {

  private MesosMasterNodeTemplate template = new MesosMasterNodeTemplate();
  ...
  private Map<String, String> createCloudConfigProperties() {
    return MesosMasterNodeTemplate.createProperties(MASTER_COUNT, createZookeeperAddresses());
  }
  ...
  public class CreateUserDataTemplateTest {
    ...
    public void testSuccess() {
      ...
      String zkQuorum = userData.parameters.get("$ZK_QUORUM");
      assertEquals(zkQuorum, ZOOKEEPER_QUORUM_STRING);

      String mesosPort = userData.parameters.get("$MESOS_PORT");
      assertEquals(mesosPort, Integer.toString(ServicesManagerConstants.Mesos.MESOS_PORT));

      String quorumSize = userData.parameters.get("$QUORUM_SIZE");
      assertEquals(quorumSize, "2");
    }
  ...
  public class CreatePropertiesTest {
    ...
    public void testZeroMasterCount() {
      MesosMasterNodeTemplate.createProperties(0, createZookeeperAddresses());
    }

    @Test(expectedExceptions = IllegalArgumentException.class)
    public void testNegativeMasterCount() {
      MesosMasterNodeTemplate.createProperties(0, createZookeeperAddresses());
    }

    @Test(expectedExceptions = NullPointerException.class)
    public void testNullZookeeperAddresses() {
      MesosMasterNodeTemplate.createProperties(2, null);
    }

    @Test(expectedExceptions = IllegalArgumentException.class)
    public void testEmpltyZookeeperAddresses() {
      MesosMasterNodeTemplate.createProperties(2, new ArrayList<>());
    }
  }
}
