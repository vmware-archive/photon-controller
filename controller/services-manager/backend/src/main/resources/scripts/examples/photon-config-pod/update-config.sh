...
# User is responsible for creating a secret called 'photon-keys' with username/password in it.
...
# As soon as secret is created, this script will fetch the secret
# and update the $PHOTON_CONF_FILE with it.

PHOTON_CONF_FILE="/etc/photon/photon.cfg"
PHOTON_BASE_FILE="./photon.cfg"
...
while true
do
  # If photon.cfg is not available at desired destination then copy our default.
