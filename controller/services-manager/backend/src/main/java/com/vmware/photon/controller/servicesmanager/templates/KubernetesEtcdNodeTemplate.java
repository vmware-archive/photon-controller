...
public class KubernetesEtcdNodeTemplate implements NodeTemplate {
  ...
  public FileTemplate createUserDataTemplate(String scriptDirectory, Map<String, String> properties) {
    ...
    parameters.put("$ETCD_PORT", Integer.toString(ServicesManagerConstants.Swarm.ETCD_PORT));
    parameters.put("$ETCD_PEER_PORT", Integer.toString(ServicesManagerConstants.Swarm.ETCD_PEER_PORT));
  ...
  private static String createEtcdParameters(List<String> etcdIps) {
    ...
    for (int i = 0; i < etcdIps.size(); i++) {
      sb.append("etcd" + i + "=http://" + etcdIps.get(i) + ":" + ServicesManagerConstants.Swarm.ETCD_PEER_PORT);
      if (i != etcdIps.size() - 1) {
        sb.append(",");
      }
    }
  ...
  private static String createEtcdPeerUrl(String ip) {
    return "http://" + ip + ":" + ServicesManagerConstants.Swarm.ETCD_PEER_PORT;
  }

  private static String createEtcdAdvertiseUrl(String ip) {
    return "http://" + ip + ":" + ServicesManagerConstants.Swarm.ETCD_PORT;
  }
}
