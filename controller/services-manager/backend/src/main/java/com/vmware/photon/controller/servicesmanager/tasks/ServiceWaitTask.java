...
import com.vmware.photon.controller.servicesmanager.statuschecks.StatusChecker;
 ...
 */
public class ServiceWaitTask extends StatefulService {

  public ServiceWaitTask() {
    super(State.class);
  ...
  private void processTask(final State currentState, Boolean isReady) {
    if (!isReady) {
      ...
      if (currentState.apiCallPollIterations >= currentState.maxApiCallPollIterations) {
        failTask(new IllegalStateException("Wait period expired for service with id " + currentState.serviceId));
        return;
      }
  ...
  private void sendSelfPatch(State patchState) {
    ...
    sendRequest(Operation
        ...
        .setCompletion((Operation operation, Throwable throwable) -> {
          if (null != throwable) {
            ServiceUtils.logWarning(service, "Failed to send self-patch for service: %s", throwable.toString());
          }
        }));
  }
  ...
  protected StatusCheckHelper getStatusCheckHelper() {
    ...
    return servicesManagerFactory.createStatusCheckHelper();
  }

  /**
   * This class represents the document state associated with a {@link ServiceWaitTask} task.
   */
  ...
  public static class State extends ServiceDocument {
    ...
    /**
     * This value represents the ID of the current service.
     */
