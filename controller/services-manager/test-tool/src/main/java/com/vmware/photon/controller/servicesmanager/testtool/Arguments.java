...
public class Arguments {
  ...
  private static final String SERVICE_TYPE_ATTR_NAME = "serviceType";
  private static final String SERVICE_COUNT_ATTR_NAME = "serviceCount";
  ...
  public ServiceType getServiceType() {
    return Enum.valueOf(ServiceType.class, this.serviceType);
  }
  ...
  public static Arguments parseArguments(String[] args) throws ArgumentParserException {
    ...
    try {
      ...
      returnVal.serviceType = res.getString(SERVICE_TYPE_ATTR_NAME);
      returnVal.serviceCount = Integer.parseInt(res.getString(SERVICE_COUNT_ATTR_NAME));
  ...
  private static ArgumentParser buildArgumentParser() {
    ArgumentParser parser = ArgumentParsers.newArgumentParser("service-mgr-test-tool")
        .defaultHelp(true)
        .description("Service Manager Test Tool");

    parser.addArgument("--serviceType", "-t")
        .dest(SERVICE_TYPE_ATTR_NAME)
        .required(true)
        .help("Type of the service to create KUBERNETES, MESOS, SWARM or Harbor");

    parser.addArgument("--serviceCount", "-c")
        .dest(SERVICE_COUNT_ATTR_NAME)
        .required(true)
        .help("number of services to be created");

    parser.addArgument("--slaveCount", "-s")
        .dest(SLAVE_COUNT_ATTR_NAME)
        .required(false)
        .setDefault(100)
        .help("number of slaves to be created per service");
