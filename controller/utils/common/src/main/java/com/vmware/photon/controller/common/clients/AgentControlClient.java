...
import com.vmware.photon.controller.agent.gen.UpdateConfigRequest;
 ...
 * <p>
 * Instances of AgentControlClient, HostClient and SchedulerClient reuses and
...
public class AgentControlClient {
  ...
  private static final long UPDATE_CONFIG_TIMEOUT_MS = 60000;
  ...
  public void provision(
    ...
    provisionRequest.setAuth_enabled(authEnabled);
  ...
  public ProvisionResponse provision(
    ...
    provision(dataStoreList, imageDataStores, usedForVMs, hostAddress, hostPort,
        memoryOverCommit, statsPluginConfig, managementOnly, hostId, deploymentId,
        ntpEndpoint, authEnabled, syncHandler);
    syncHandler.await();
  ...
  public void updateConfig(
    ...
    updateConfigRequest.setImage_datastores(imageDatastoreSet);

    clientProxy.setTimeout(UPDATE_CONFIG_TIMEOUT_MS);
    logger.info("update config target: {}, request {}", getHostIp(), updateConfigRequest);

    try {
      clientProxy.update_config(updateConfigRequest, handler);
    } catch (TException e) {
      throw new RpcException(e.getMessage());
    }
  }
  ...
  /**
   * This method performs a synchronous Thrift call to update agent's configuration.
   *
   * @return On success, the return code is the UpdateConfigResponse object
   * generated by the Thrift call.
   * @throws InterruptedException
   * @throws RpcException
   */
  @RpcMethod
  public UpdateConfigResponse updateConfig(Set<String> imageDataStores, boolean usedForVMs)
      throws InterruptedException, RpcException {
    SyncHandler<UpdateConfigResponse, AgentControl.AsyncSSLClient.update_config_call> syncHandler = new SyncHandler<>();
    updateConfig(imageDataStores, usedForVMs, syncHandler);
    syncHandler.await();
    return ResponseValidator.checkUpdateConfigResponse(syncHandler.getResponse());
  }
  ...
  public static class ResponseValidator {
    ...
    /**
     * This method validates a UpdateConfigResponse object, raising an exception if
     * the response reflects an operation failure.
     *
     * @param updateConfigResponse Supplies a UpdateConfigResponse object generated by
     *                          a update_config call.
     * @return On success, the return value is the UpdateConfigResponse object
     * specified as a parameter.
     * @throws RpcException
     */
    public static UpdateConfigResponse checkUpdateConfigResponse(UpdateConfigResponse updateConfigResponse)
        throws RpcException {
      logger.info("Checking {}", updateConfigResponse);
      switch (updateConfigResponse.getResult()) {
        case OK:
          break;
        case SYSTEM_ERROR:
          throw new SystemErrorException(updateConfigResponse.getError());
        default:
          throw new RpcException(String.format("Unknown result: %s", updateConfigResponse.getResult()));
      }
