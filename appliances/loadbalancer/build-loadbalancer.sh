...
mkdir -p ./build
rm -rf ./build/*
packer build -force loadbalancer.json

# Make OVA VMware compatible
cd build
${SCRIPT_DIR}/../scripts/toVMwareOva.sh loadbalancer-virtualbox loadbalancer ${SCRIPT_DIR}/add-ovf-params.sh
...
popd
mv loadbalancer.ova loadbalancer-${PC_VERSION}-${COMMIT}.ova
