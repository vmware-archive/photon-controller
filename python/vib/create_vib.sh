...
chmod a+w $SRC_VIB_LAYOUT/payloads/agent/etc/opt/vmware/photon/controller/{no-auth,rui}.pem
chmod +t $SRC_VIB_LAYOUT/payloads/agent/etc/opt/vmware/photon/controller/{no-auth,rui}.json
...
build_for_py_ver() {
   ...
   # our ci environments do not provide access to the git repository so these variables
	 # won't be properly populated
	 if [[ $GERRIT_BRANCH != *[!\ ]* ]]; then GERRIT_BRANCH="unkown"; fi
	 if [[ $COMMIT_HASH != *[!\ ]* ]]; then COMMIT_HASH="unkown"; fi
