...
class DirectClient(object):
    ...
    def __init__(self, service_name, client_cls, host, port, client_timeout=None,
                 certfile=None, capath=None, validate=True):
    ...
    def connect(self):
        ...
        self._logger.info("Initialize SSLSocket using cert=%s, capath=%s" % (self._certfile, self._capath))
        sock = TSSLSocket.TSSLSocket(host=self._host, port=self._port, validate=self._validate,
                                     certfile=self._certfile, capath=self._capath)
