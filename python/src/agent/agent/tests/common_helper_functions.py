...
def _wait_for_configuration(client, num_children, num_retries=20):
    """
    Poll the list of schedulers from root scheduler

    client - thrift client to the root scheduler
    num_children - keep trying until you have this many children
    num_retries - maximum number of retries
    """
    for i in xrange(num_retries):
        try:
            res = client.get_schedulers()
            role = res.schedulers[0].role
            if (role.scheduler_children and
               len(role.scheduler_children) == num_children):
                return role
        except:
            pass
        time.sleep(1)
    raise Exception("did not get configured within % sec" % num_retries)
...
class RuntimeUtils(object):
    ...
    def start_root_scheduler(self, configs=None):
        ...
        root_config.update(configs)
        self._configure_logging(root_config, self.ROOT_SCHEDULER)
        with tempfile.NamedTemporaryFile(delete=False) as conf:
            with open(conf.name, 'w+') as f:
                f.write(yaml.dump(root_config, default_flow_style=False))
