...
class TestRemoteAgent(BaseKazooTestCase, AgentCommonTests):
    ...
    def setUp(self):
        ...
        if "chairman" in config["agent_remote_test"]:
            chairman = config["agent_remote_test"]["chairman"].split(":")
            self._chairman = ServerAddress(chairman[0], int(chairman[1]))
            self._chairman_specified = True
        else:
            self._chairman = ServerAddress("localhost", 13000)
            self._chairman_specified = False
    ...
    def test_send_image_to_host(self):
        ...
        transfer_image_request = TransferImageRequest(
            ...
            destination_datastore=datastore.name,
    ...
    def _delete_network(self, network_system, network_name, vswitch_name):
        try:
            network_system.RemovePortGroup(pgName=network_name)
