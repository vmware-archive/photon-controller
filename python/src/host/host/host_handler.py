        ...
        for disk in vm.disks:
            if disk.image is None:
                ...
                if not os.path.exists(parent_vmdk_os_path):
                    ...
                    image_dir = os.path.dirname(parent_vmdk_os_path)
                    if os.path.exists(image_dir):
                        self._logger.debug("longz.longz image folder %s exist" % image_dir)
                        self._logger.debug("longz.longz image folder %s content: %s" %
                                           image_dir, str(os.listdir(image_dir)))
