...
class EsxVmConfig(object):
    ...
    def _add_disk(self, cfg_spec, datastore, disk_id, controller_key,
        ...
        if disk_id:
            ...
            # used when importing a VM, vmdk path is really just a
            # placeholder of a disk that is destined for a folder in
            # datastore specified.
            vmdk_file = '[%s]' % datastore
    ...
    def add_scsi_disk(self, cfg_info, cfg_spec, datastore, disk_id,
        ...
        # when disk_id is not specified, it is assumed we will be specifying
        # to created a disk
        size_mb = None if disk_id else 1
        create = False if disk_id else True
        self._add_disk(cfg_spec, datastore, disk_id, controller.key,
                       disk_root_folder=folder,
                       create=create, size_mb=size_mb)  # XXX
