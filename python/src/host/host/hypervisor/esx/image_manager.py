...
class EsxImageManager(ImageManager):
    ...
    def reap_tmp_images(self):
        ...
        for ds in self._ds_manager.get_datastores():
            ...
            for images_dir in glob.glob(tmp_image_pattern):
                if not os.path.isdir(images_dir):
                    continue

                create_time = os.stat(images_dir).st_ctime
                ...
                if current_time - self.REAP_TMP_IMAGES_GRACE_PERIOD < create_time:
                    ...
                    self._logger.info(
                        ...
                        (images_dir, create_time, current_time))
                    continue

                try:
                    with FileBackedLock(images_dir, ds.type):
                        if (os.path.exists(images_dir)):
                            self._logger.info("Delete folder %s" % images_dir)
                            shutil.rmtree(images_dir, ignore_errors=True)
                except (AcquireLockFailure, InvalidFile):
                    self._logger.info("Already locked: %s, skipping" % images_dir)
                except:
                    self._logger.info("Unable to remove %s" % images_dir, exc_info=True)
