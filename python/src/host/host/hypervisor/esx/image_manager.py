...
from host.hypervisor.esx.vm_config import COMPOND_DIRECTORY_SEPARATOR
...
class EsxImageManager(ImageManager):
    ...
    def get_images(self, datastore):
        ...
        if SUPPORT_VSAN:
            ...
            for dir in glob.glob(image_folder_pattern):
                image_id = dir.split(COMPOND_DIRECTORY_SEPARATOR)[1]
    ...
    def get_image_id_from_path(self, image_path):
        ...
        """
        if SUPPORT_VSAN:
            return image_path.split(os.sep)[4].split(COMPOND_DIRECTORY_SEPARATOR)[1]
        else:
