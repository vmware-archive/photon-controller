...
class VimSessionOrientedStub(SessionOrientedStub):
    ...
    def makeUserLoginMethod(username, password, locale=None):
        '''Return a function that will call the vim.SessionManager.Login()
...
def __GetServiceVersionDescription(protocol, server, port, path):
    """
    Private method that returns an ElementTree describing the API versions
    supported by the specified server. The result will be
    vimServiceVersions.xml if it exists, otherwise vimService.wsdl if it
    exists, otherwise None.

    @param protocol: What protocol to use for the connection (e.g. https or
                     http).
    ...
    tree = ElementTree()
    try:
        socket = urllib.urlopen("%s://%s:%s/%s/vimServiceVersions.xml" %
        ...
        socket.close()
    try:
        socket = urllib.urlopen("%s://%s:%s/%s/vimService.wsdl" %
                                (protocol, server, port, path))
        tree.parse(socket)
        socket.close()
        return tree
...
def __VersionIsSupported(desiredVersion, serviceVersionDescription):
    ...
    if root.tag == 'namespaces':
        ...
        importElement = serviceVersionDescription.find(
            './/{%s}import' % wsdlNS)
...
def __FindSupportedVersion(protocol, server, port, path, preferredApiVersions):
    ...
    @param path: Path
    @type  path: string
    @param preferredApiVersions: Acceptable API version(s) (e.g.
                                 vim.version.version3) If a list of versions is
                                 specified the versions should be ordered from
...
def SmartConnect(protocol='https', host='localhost', port=443, user='root',
                 pwd='',
                 service="hostd", path="/sdk",
                 preferredApiVersions=None):
    """
    Determine the most preferred API version supported by the specified server,
    then connect to the specified server using that API version, login and
    ...
    @param preferredApiVersions: Acceptable API version(s) (e.g.
                                 vim.version.version3) If a list of versions is
                                 specified the versions should be ordered from
                                 most to least preferred. If None is specified,
                                 the list of versions support by pyVmomi will
                                 be used.
    @type  preferredApiVersions: string or string list
    """
