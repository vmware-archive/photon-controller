...
class TestVimClient(unittest.TestCase):
    ...
    def test_initialize_host_counters(self):
        self.vim_client._initialize_stats_defaults()
        assert_that(self.vim_client._stats_counters_initialized, equal_to(False))
        ...
        self.vim_client.initialize_stats_counters(metric_names, sampling_interval)
        assert_that(self.vim_client._stats_counters_initialized, equal_to(True))
        assert_that(self.vim_client._stats_sampling_interval, equal_to(sampling_interval))
        assert_that(len(self.vim_client._stats_metric_id_objs), equal_to(len(metric_names)))
        assert_that(len(self.vim_client._stats_counter_to_metric_map), equal_to(len(metric_names)))
    ...
    def test_query_stats(self):
            ...
            vim.PerformanceManager.EntityMetricCSV(
                ...
                value=[
                    vim.PerformanceManager.MetricSeriesCSV(
                        id=vim.PerformanceManager.MetricId(counterId=6567,
                                                           instance=''),
                        value='200')]
            ),
            vim.PerformanceManager.EntityMetricCSV(
                ...
                value=[
                    vim.PerformanceManager.MetricSeriesCSV(
                        id=vim.PerformanceManager.MetricId(counterId=6669,
                                                           instance=''),
                        value='200')]
            ),
        ]
        ...
        self.vim_client.initialize_stats_counters(metric_names, 20)
        results = self.vim_client.query_stats(host, since, None)
