...
class TestHttpTransfer(unittest.TestCase):
    ...
    def setUp(self, connect, creds):
        self.shadow_vm_id = "shadow_%s" % str(uuid.uuid1())
    ...
    def test_export_shadow_vm(self):
        ...
        shadow_vm_id = "shadow_%s" % str(uuid.uuid1())
        lease, url = self.http_transferer._export_shadow_vm(shadow_vm_id)

        mock_get_vm.assert_called_once_with(shadow_vm_id)
