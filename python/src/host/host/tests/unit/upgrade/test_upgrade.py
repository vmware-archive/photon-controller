...
class ImageSweeperTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.host_upgrade = HostUpgrade(self.datastore_manager, "/tmp")

        self.timeout = self.TIMEOUT

    def tearDown(self):
        self.host_upgrade.stop()
    ...
    def test_lifecycle(self, create_symlinks):
        ...
        self.host_upgrade.set_upgrade_needed()
        self.assertTrue(os.path.exists(self.host_upgrade._marker_file_path))

        self.host_upgrade.start(self.timeout)

        self.host_upgrade.wait_for_task_end()

        assert_that(not self.host_upgrade.in_progress())
        self.assertTrue(not os.path.exists(self.host_upgrade._marker_file_path))

        self.assertEqual(create_symlinks.call_count, 2)

    @patch("host.upgrade.softlink_generator.SoftLinkGenerator.create_symlinks_to_new_image_path")
    def test_upgrade_not_needed(self, create_symlinks):
        assert_that(not self.host_upgrade.in_progress())
        ...
        self.assertEqual(create_symlinks.call_count, 0)
