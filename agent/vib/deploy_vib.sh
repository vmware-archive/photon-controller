...
sshpass -p "$ESX_PWD" ssh $SSH_OPTS $target_host "esxcli software vib remove --vibname=${vibname} -f || true"
sshpass -p "$ESX_PWD" ssh $SSH_OPTS $target_host "esxcli software vib install -v /tmp/${vibfilename} -f"
