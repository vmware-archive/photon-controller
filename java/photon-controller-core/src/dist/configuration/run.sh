...
LIGHTWAVE_DISABLE_VMAFD_LISTNER=`get_config_value $PHOTON_CONTROLLER_CORE_CONFIG lightwaveDisableVmafdListner:`
...
print_warning_if_value_mssing "${LIGHTWAVE_DISABLE_VMAFD_LISTNER}" "lightwaveDisableVmafdListner"    "$PHOTON_CONTROLLER_CORE_CONFIG"
...
then
  ...
  then
    ic_join_params="$ic_join_params --machine-account ${LIGHTWAVE_MACHINE_ACCOUNT}"
  fi

  if [ "${LIGHTWAVE_DISABLE_VMAFD_LISTNER}" == "True" ]
  then
    ic_join_params="$ic_join_params --disable-vmafd-listner true"
  fi
