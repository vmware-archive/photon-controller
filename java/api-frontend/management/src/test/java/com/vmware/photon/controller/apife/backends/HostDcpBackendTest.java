...
public class HostDcpBackendTest {
  ...
  public static class QueryHostTest {
    ...
    public void testFilterHost() throws Throwable {
      ...
      ResourceList<Host> hosts = hostBackend.listAll(Optional.of(2));
      assertThat(hosts, notNullValue());
      assertThat(hosts.getItems().size(), is(2));

      hosts = hostBackend.filterByUsage(UsageTag.CLOUD, Optional.of(2));
      assertThat(hosts.getItems().size(), is(2));

      hosts = hostBackend.filterByUsage(UsageTag.IMAGE, Optional.of(2));
      assertThat(hosts.getItems().size(), is(1));

      hosts = hostBackend.filterByUsage(UsageTag.MGMT, Optional.of(2));
