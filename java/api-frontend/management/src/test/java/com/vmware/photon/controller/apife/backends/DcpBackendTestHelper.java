...
/**
 * Common helper methods for use by DcpBackend test classes.
 */
public class DcpBackendTestHelper {

  public static String createTenant(TenantDcpBackend tenantDcpBackend, String tenantName) throws ExternalException {
    TenantCreateSpec tenantSpec = new TenantCreateSpec();
    tenantSpec.setName(tenantName);
    TaskEntity tenantTask = tenantDcpBackend.createTenant(tenantSpec);
    return tenantTask.getEntityId();
  }

  public static String createTenantResourceTicket(ResourceTicketDcpBackend resourceTicketDcpBackend,
                                                  String tenantId,
                                                  String name,
    ...
    return resourceTicketDcpBackend.create(tenantId, resourceTicketSpec).getId();
  }

  public static String createProject(ProjectDcpBackend projectDcpBackend,
                                     String projectName,
                                     String tenantId,
    ...
    TaskEntity projectTask = projectDcpBackend.createProject(tenantId, projectSpec);
    return projectTask.getEntityId();
  }

  public static void createFlavors(FlavorDcpBackend flavorDcpBackend,
                                   List<FlavorCreateSpec> flavorCreateSpecs) throws ExternalException {
    for (FlavorCreateSpec spec : flavorCreateSpecs) {
      try {
        flavorDcpBackend.getEntityByNameAndKind(spec.getName(), spec.getKind());
        ...
        flavorDcpBackend.createFlavor(spec);
      }
    }
  }
}
