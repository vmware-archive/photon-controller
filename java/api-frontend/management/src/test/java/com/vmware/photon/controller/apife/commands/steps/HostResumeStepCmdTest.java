...
/**
 * Test {@link HostResumeStepCmd}.
 */
public class HostResumeStepCmdTest extends PowerMockTestCase {
  ...
  private HostResumeStepCmd command;
  ...
  public void setUp() {
    ...
    command = new HostResumeStepCmd(taskCommand, stepBackend, step, hostBackend);
    deployerClient = mock(DeployerClient.class);
    when(taskCommand.getDeployerClient()).thenReturn(deployerClient);
  }

  @Test
  public void testSuccessfulHostResume() throws Exception {
  ...
  public void testRpcException() throws Exception {
    when(deployerClient.enterNormalMode(hostEntity.getId())).
        thenThrow(new RpcException("Failed to resume host to normal mode with Rpc exception."));
    try {
      command.execute();
      fail("should have failed with RpcException.");
    } catch (InternalException e) {
      assertThat(e.getMessage(), containsString("Failed to resume host to normal mode with Rpc exception."));
    }
    verify(deployerClient).enterNormalMode(hostEntity.getId());
  ...
  public void testRuntimeException() throws Exception {
    when(deployerClient.enterNormalMode(hostEntity.getId())).
        thenThrow(new RuntimeException("Failed to resume host to normal mode with Runtime exception."));
    try {
      command.execute();
      fail("should have failed with RuntimeException.");
    } catch (RuntimeException e) {
      assertThat(e.getMessage(), containsString("Failed to resume host to normal mode with Runtime exception."));
    }
    verify(deployerClient).enterNormalMode(hostEntity.getId());
    verify(hostBackend, never()).updateState(hostEntity, HostState.READY);
  }
