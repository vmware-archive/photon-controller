...
/**
 * API to support getting all tasks related to a given host.
 */
@Path(HostResourceRoutes.HOST_TASKS_PATH)
@Api(value = HostResourceRoutes.API)
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class HostTasksResource {

  private final TaskFeClient taskFeClient;

  @Inject
  public HostTasksResource(TaskFeClient taskFeClient) {
    this.taskFeClient = taskFeClient;
  }

  @GET
  @ApiOperation(value = "Find tasks associated with a Host. If pageLink is provided, " +
      "then get the tasks on that specific page", response = Task.class, responseContainer = ResourceList.CLASS_NAME)
  @ApiResponses(value = {
      @ApiResponse(code = 200, message = "List of tasks for a host")
  })
  public Response get(@Context Request request,
    ...
    return generateResourceListResponse(
        Response.Status.OK,
        DataTypeConversionUtils.formalizePageLinks(resourceList, HostResourceRoutes.API),
