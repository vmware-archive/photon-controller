...
public class DatastoreDcpBackend implements DatastoreBackend{
  ...
  @Override
  public ResourceList<Datastore> filter(Optional<String> tag, Optional<Integer> pageSize) {
  ...
  private DatastoreService.State findById(String id) throws DatastoreNotFoundException {
    try {
      com.vmware.xenon.common.Operation result = dcpClient.get(DatastoreServiceFactory.SELF_LINK + "/" + id);
