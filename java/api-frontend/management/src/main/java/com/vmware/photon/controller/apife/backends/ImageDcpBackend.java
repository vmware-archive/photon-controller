...
public class ImageDcpBackend implements ImageBackend {
  ...
  public void updateImageDatastore(String imageId, String imageDatastoreName) throws ExternalException {
    ...
    //Query to see if the ImageReplicationService already existed for the same imageDatastoreId and imageId
    final ImmutableMap.Builder<String, String> termsBuilderImageReplicationService = new ImmutableMap.Builder<>();
    termsBuilderImageReplicationService.put("imageId", imageId);
    termsBuilderImageReplicationService.put("imageDatastoreId", datastores.get(0).id);
    List<ImageReplicationService.State> imageReplicationServiceStates = dcpClient.queryDocuments(ImageReplicationService.State
            .class, termsBuilderImageReplicationService.build());
    if (imageReplicationServiceStates.size() > 0) {
      return;
    }
