...
/**
 * API to support getting all tasks related to a given flavor.
 */
@Path(FlavorsResourceRoutes.FLAVOR_TASKS_PATH)
@Api(value = FlavorsResourceRoutes.API)
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class FlavorTasksResource {

  private final TaskFeClient taskFeClient;

  @Inject
  public FlavorTasksResource(TaskFeClient taskFeClient) {
    this.taskFeClient = taskFeClient;
  }

  @GET
  @ApiOperation(value = "Find tasks associated with a Flavor. If page link is provided, " +
      "then get the tasks on that specific page", response = Task.class, responseContainer = ResourceList.CLASS_NAME)
  @ApiResponses(value = {
      @ApiResponse(code = 200, message = "List of tasks for a flavor")
  })
  public Response get(@Context Request request,
    ...
    ResourceList<Task> resourceList = pageLink.isPresent()
        ...
        : taskFeClient.getFlavorTasks(id, state, pageSize);

    return generateResourceListResponse(
        Response.Status.OK,
        DataTypeConversionUtils.formalizePageLinks(resourceList, FlavorsResourceRoutes.API),
