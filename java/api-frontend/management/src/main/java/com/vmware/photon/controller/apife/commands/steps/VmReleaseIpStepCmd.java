...
public class VmReleaseIpStepCmd extends StepCommand {
  ...
  @Override
  protected void execute() throws ApiFeException, InterruptedException, RpcException, ExternalException {
    ...
    for (VmService.NetworkInfo networkInfo : networkInfoMap.values()) {
      patchIpAllocation(networkInfo.macAddress);
    }
  ...
  private void patchIpAllocation(String macAddress) {
    ...
    photonControllerXenonRestClient.patch(IpAllocatorService.FACTORY_LINK, patch);
  }
}
