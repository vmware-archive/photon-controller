...
/**
 * This resource is for disk detach related API.
 */
@Path(VmResourceRoutes.VM_DETACH_DISK_PATH)
@Api(value = VmResourceRoutes.API)
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class VmDiskDetachResource {

  private final VmFeClient vmFeClient;

  @Inject
  public VmDiskDetachResource(VmFeClient vmFeClient) {
    this.vmFeClient = vmFeClient;
  }

  @POST
  @ApiOperation(value = "Detaches a PERSISTENT_DISK from the VM", response = Task.class)
  @ApiResponses(value = {
      @ApiResponse(code = 201, message = "VM state is changing, progress communicated via the task")
  })
  public Response operate(@Context Request request, @PathParam("id") String id,
                          @Validated VmDiskOperation opSpec) throws ExternalException {
    List<String> diskIds = new ArrayList<>();
    diskIds.add(opSpec.getDiskId());
    return generateCustomResponse(Response.Status.CREATED,
        vmFeClient.operateDisks(id, diskIds, Operation.DETACH_DISK),
