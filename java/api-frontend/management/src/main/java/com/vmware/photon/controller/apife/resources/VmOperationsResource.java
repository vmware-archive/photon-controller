...
/**
 * This resource is for vm operation related API.
 */
@Path(VmResourceRoutes.VM_OPERATIONS_PATH)
@Api(value = VmResourceRoutes.API)
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class VmOperationsResource {

  private final VmFeClient vmFeClient;

  @Inject
  public VmOperationsResource(VmFeClient vmFeClient) {
    this.vmFeClient = vmFeClient;
  }

  @POST
  @ApiOperation(value = "Perform a state change operation on a VM", response = Task.class)
  @ApiResponses(value = {
  ...
  public Response operate(@Context Request request,
                          ...
                          @Validated VmOperation opSpec) throws ExternalException {

    Operation operation = Operation.valueOf(opSpec.getOperation());
    return generateCustomResponse(
        Response.Status.CREATED,
        vmFeClient.operate(id, operation),
