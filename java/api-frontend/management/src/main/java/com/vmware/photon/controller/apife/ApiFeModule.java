...
public class ApiFeModule extends AbstractModule {
  ...
  private void bindExtensions() {
    String className = Implementations.nameToPath.get(Implementations.ExtensionName.VM_MIGRATION);
    Class<?> clazz = loadExtension(Implementations.ExtensionName.VM_MIGRATION, className);
    bind(VmMigration.class).to((Class<? extends VmMigration>) clazz);
  }
