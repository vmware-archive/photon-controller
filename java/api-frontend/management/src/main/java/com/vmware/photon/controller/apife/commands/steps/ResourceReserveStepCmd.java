...
public class ResourceReserveStepCmd extends StepCommand {
  ...
  private static final long PLACEMENT_RETRY_INTERVAL = TimeUnit.SECONDS.toMillis(2);
  ...
  private String loadReservation(Resource resource, String targetHostIp)
    ...
    while (true) {
      try {
        ...
        if (retries > 1) {
          // we retry the first time right away and then we sleep between subsequent retries to give the scheduling
          // tree some time to stabilize.
