      ...
      if (!documents.isEmpty()) {
        ...
        for (Map.Entry<?, ?> entry : documents.entrySet()) {
          try {
            ...
            // The 2nd arg represents the default level, but if a valid debug level
            // string isn't passed in we want to return a failure, not silently
            // change it to some other level
            logLevel = Level.toLevel(logLevelString, null);

            if (logLevel == null) {
              throw new Exception("Invalid log level " + logLevelString);
            }
