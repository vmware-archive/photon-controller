...
public class ImageCopyService extends StatefulService {
  ...
  private void getHostFromDataStore(final State current) {
    convertDatastoreNameToId(current);
  ...
  private void convertDatastoreNameToId (final State current) {
    QueryTask.Query kindClause = new QueryTask.Query()
        .setTermPropertyName(ServiceDocument.FIELD_NAME_KIND)
        .setTermMatchValue(Utils.buildKind(DatastoreService.State.class));

    String fieldName = QueryTask.QuerySpecification.buildCollectionItemName("name");
    QueryTask.Query imageDatastoreClause = new QueryTask.Query()
        .setTermPropertyName(fieldName)
        ...
        ((HousekeeperDcpServiceHost) getHost()).getCloudStoreHelper()
            ...
            .setCompletion(
                (operation, throwable) -> {
                  ...
                  checkState(documentLinks.size() > 0, "No image datastore has the name");
                  checkState(documentLinks.size() == 1, "more than one image datastore has the same name");
