    ...
    for (InactiveImageDescriptor image : inactiveImages) {
      ...
      if (null != referenceImage) {
        ...
        if (referenceImage.state == ImageState.PENDING_DELETE) {
          ServiceUtils.logInfo(this, "Image: %s is in pending delete: %s", image.getImage_id(), referenceImage.state);
        ...
        if (referenceImage.replicationType == ImageReplicationType.EAGER) {
          ServiceUtils.logInfo(this, "Image: %s is EAGER", image.getImage_id());
        ...
        if (current.isImageDatastore) {
          ServiceUtils.logInfo(this, "Image: %s is not EAGER, but is on an image datastore", image.getImage_id());
          continue;
        }
      }

      ServiceUtils.logInfo(this, "Image: %s is not EAGER and not on an image datastore", image.getImage_id());
      // image is:
