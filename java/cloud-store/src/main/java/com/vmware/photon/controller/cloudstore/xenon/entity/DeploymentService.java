...
public class DeploymentService extends StatefulService {
  ...
  private void validateForSetSystemConfig(State patchState, Operation request, Action action) {
    ServiceUtils.logInfo(this, "SystemConfig update requires validation for Action: %s  PatchBody: %s " +
            "getState: %s",
        action, Utils.toJson(patchState), Utils.toJson(getState(request)));
