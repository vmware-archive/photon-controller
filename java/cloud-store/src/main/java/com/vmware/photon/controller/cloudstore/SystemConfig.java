...
public class SystemConfig implements SystemConfigProvider {
  ...
  protected SystemConfig(PhotonControllerXenonHost serviceHost) {
  ...
  public static SystemConfig getInstance(PhotonControllerXenonHost serviceHost) {
    if (instance == null) {
      instance = new SystemConfig(serviceHost);
      serviceHost.setSystemConfigProvider(instance);
    }
    return instance;
  }
  ...
  private class PausedStateChecker implements Runnable {
    public void run() {
      ...
      try {
        CloudStoreHelper cloudStoreHelper = xenonHost.getCloudStoreHelper();
