...
public class SchedulingConstantGenerator extends StatefulService {
  ...
  public static final String SINGLETON_LINK = FACTORY_LINK + "/seq";
  ...
  /**
   * Synchronously create a singleton SchedulingConstantGenerator for unit tests.
   *
   * Creates a new singleton SchedulingConstantGenerator instance, synchronously.
   * This is provided as a simple way to initialize the singleton instance for
   * unit tests. Do not use this method outside of unit tests.
   *
   ...
   * cleanup.
   */
  public static void startSingletonServiceForTest(ServiceHost host) throws Throwable {
    SchedulingConstantGenerator.State newState = new SchedulingConstantGenerator.State();
    newState.documentSelfLink = SchedulingConstantGenerator.SINGLETON_LINK;
    ...
    ServiceHostUtils.sendRequestAndWait(host, postOperation, postOperation.getRefererAsString());
  }
  ...
  private void nextValue(State state) {
    ...
    if (state.index == null) {
      throw new RuntimeException("state.index == null");
    }
  ...
  private void validateState(State state) {
    ServiceUtils.logInfo(this, "Validating state for service %s", getSelfLink());
