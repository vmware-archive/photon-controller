...
public class HaltonSequenceService extends StatefulService {
  ...
  /**
   * Creates a POST request that creates a singleton HaltonSequenceService. Does
   * not send the request.
   */
  private static Operation createPostToStartSingleton(ServiceHost host) {
  ...
  public static Operation.CompletionHandler startSingletonService(ServiceHost host) {
    return (o, e) -> {
      ...
      Operation post = createPostToStartSingleton(host);
      host.sendRequest(post);
    };
  }

  /**
   * Start the HaltonSequenceService factory and singleton instance on each
   * Xenon host, synchronously. This function is provided to simplify
   * initialization in unit tests that depend on HaltonSequenceService to create
   * HostService instances.
   * @param hosts Xenon ServiceHosts on which to start a HaltonSequenceService.
   */
  public static void startSingletonServiceForTest(ServiceHost[] hosts) throws InterruptedException, TimeoutException, BadRequestException, DocumentNotFoundException {
    // HaltonSequenceService is non-owner-selected: it needs to be started on
    // each Xenon host separately
    for (ServiceHost host: hosts) {
      host.startFactory(HaltonSequenceService.class, HaltonSequenceService::createFactory);
      Operation post = createPostToStartSingleton(host);
      ServiceHostUtils.sendRequestAndWait(host, post, post.getRefererAsString());
    }
  }
