...
public class HostService extends StatefulService {
   ...
   */
  public static final int UPDATE_TIMEOUT_MICRO_SECONDS = 5 * 1000000;
  ...
  public void handlePatch(Operation patchOperation) {
    ...
    try {
      ...
      // Whenever the host state changes to READY, we explicitly
      // update host configuration by calling into the agent. This
      // is an optimization to make sure the host service reflects
      // the latest state of the host. In normal cases,
      // handleMaintenance will perform this update periodically.
  ...
  private void processHostConfig(Operation operation, State hostState, HostConfig hostConfig) {
    try {
      ...
      setDatastoreState(operation, hostState, datastores, imageDatastoreIds);
    } catch (Throwable ex) {
  ...
  private void setDatastoreState(
      Operation operation, HostService.State hostState, List<Datastore> datastores, Set<String> imageDatastores) {
    if (datastores != null) {
      OperationJoin
          .create(datastores.stream().map((datastore) -> {
            ...
            return Operation
                ...
                .setExpiration(UPDATE_TIMEOUT_MICRO_SECONDS);
          }))
