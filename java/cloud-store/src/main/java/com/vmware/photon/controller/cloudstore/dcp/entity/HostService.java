...
public class HostService extends StatefulService {
   ...
   * state (15 seconds).
   */
  public static final long DEFAULT_MAINTENANCE_INTERVAL_MILLIS = 15 * 1000;
   ...
   */
  public static final int DEFAULT_MAX_PING_WAIT_TIME_MILLIS = 10 * 1000;
  ...
  public void handleMaintenance(Operation maintenance) {
    Operation getOperation = Operation.createGet(this, maintenance.getUri().getPath())
        .setCompletion((op, ex) -> {
          if (ex != null) {
            ServiceUtils.logWarning(this, "Get request failed on Host Service to ping agent!");
          }
  ...
  private void pingHost(Operation maintenance, State hostState) {
    getHost().schedule(() -> {
      ...
      try {
        ...
        ServiceUtils.logInfo(this, "Failed to ping " + hostState.hostAddress + ":" + ex.getMessage());
        updateHostState(maintenance, hostState, AgentState.MISSING);
      } catch (Exception ex) {
        ServiceUtils.logWarning(this, "Unexpected exception while pinging " + hostState.hostAddress + ":" +
            ex.getMessage());
        updateHostState(maintenance, hostState, AgentState.MISSING);
      } finally {
    ...
    }, ThreadLocalRandom.current().nextInt(1, hostState.maxPingWaitTimeMillis), TimeUnit.MILLISECONDS);
  }
  ...
  private void updateHostState(Operation maintenance, State hostState, AgentState agentState) {
    if (hostState.agentState != agentState) {
      ...
      ServiceUtils.logInfo(this, "Agent state changed from " + hostState.agentState + " -> " + agentState);
  ...
  public static class State extends ServiceDocument {
    ...
    /**
     * This value represents the upper bound of the wait time in milliseconds for a host
     * service instance to ping the agent within its polling interval.
     */
    @DefaultInteger(value = DEFAULT_MAX_PING_WAIT_TIME_MILLIS)
    @Positive
    public Integer maxPingWaitTimeMillis;
  }
}
