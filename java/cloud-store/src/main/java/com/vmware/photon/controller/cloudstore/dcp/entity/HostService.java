...
public class HostService extends StatefulService {
   ...
   *
   * DEFAULT_MAINTENANCE_INTERVAL = DEFAULT_MAX_PING_WAIT_TIME + MAX_TIME_TO_PING + BUFFER = 50 + 1 + 9 = 60s
   *
  ...
  private void pingHost(Operation maintenance, State hostState) {
    ...
    try {
      TNonblockingTransport transport = new TNonblockingSocket(hostState.hostAddress, hostState.agentPort);
      TProtocolFactory protocolFactory = new TCompactProtocol.Factory();
      TProtocolFactory mProtocolFactory = new MultiplexedProtocolFactory(protocolFactory, "AgentControl");
      TAsyncClientManager clientManager = new TAsyncClientManager();
      AgentControl.AsyncClient client = new AgentControl.AsyncClient(mProtocolFactory, clientManager, transport);

      PingRequest request = new PingRequest();
      client.ping(request, new AsyncMethodCallback() {
        @Override
