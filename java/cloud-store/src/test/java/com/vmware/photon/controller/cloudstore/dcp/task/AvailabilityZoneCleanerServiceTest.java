...
/**
 * Tests {@link com.vmware.photon.controller.cloudstore.dcp.task.AvailabilityZoneCleanerService}.
 */
public class AvailabilityZoneCleanerServiceTest {
  ...
  private AvailabilityZoneCleanerService.State buildValidStartupState() {
    ...
    state.availabilityZoneExpirationAgeInMicros = 5 * 60 * 1000 * 1000L; // 5 min
    return state;
  }
  ...
  public class EndToEndTest {
    ...
    public void testSuccess(
      ...
      assertThat(response.deletedTasks, is(tasksPerAvailabilityZone * expectedAvailabilityZonesDeleted));
    ...
    public void testSuccessOnNewAvailabilityZones(
      ...
      assertThat(response.deletedTasks, is(0));
    ...
    public Object[][] getSuccessData() {
      return new Object[][]{
          ...
          {2, 2, 0, 2, 1},
          {2, 2, 2, 2, 1},
          {3, 3, 0, 2, TestEnvironment.DEFAULT_MULTI_HOST_COUNT},
          {7, 5, 2, 2, 1},
          {9, 7, 2, 1, 1},
          {9, 7, 2, 1, TestEnvironment.DEFAULT_MULTI_HOST_COUNT}
      };
    }
    ...
    private void seedTestEnvironment(TestEnvironment env,
      ...
      for (int i = 0; i < totalAvailabilityZones; i++) {
        ...
        // create tasks
        for (int j = 0; j < tasksPerAvailabilityZone; j++) {
          TaskService.State task = new TaskService.State();
          task.entityId = ServiceUtils.getIDFromDocumentSelfLink(createdAvailabilityZone.documentSelfLink);
          task.entityKind = "availability-zone";
          task.operation = com.vmware.photon.controller.api.Operation.CREATE_AVAILABILITYZONE;
          if (j % 2 > 0) {
            task.operation = com.vmware.photon.controller.api.Operation.DELETE_AVAILABILITYZONE;
          }
          task.state = TaskService.State.TaskState.COMPLETED;
          Operation taskOperation = env.sendPostAndWait(TaskServiceFactory.SELF_LINK, task);
