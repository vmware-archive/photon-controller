...
public class SchedulingConstantGeneratorTest {
  ...
  public void testPatchBody() throws Throwable {
    ...
    assertThat(state.index, notNullValue());
    assertThat(state.lastValue, notNullValue());
  ...
  public void testPatchIncrementsIndex() throws Throwable {
    TestEnvironment env = TestEnvironment.create(1);
    SchedulingConstantGenerator.State currentState =
        env.getServiceState(
            SchedulingConstantGenerator.SINGLETON_LINK,
            SchedulingConstantGenerator.State.class);
    ...
    int newIndex = patchOp.getBody(SchedulingConstantGenerator.State.class).index;
