...
public class QueryTaskUtils {
  ...
  /**
   * Gets the document links from a query result.
   *
   * @param queryResult
   * @return
   */
  public static Set<String> getQueryResultDocumentLinks(Operation queryResult) {
    NodeGroupBroadcastResponse queryResponse = queryResult.getBody(NodeGroupBroadcastResponse.class);
    return getBroadcastQueryResults(queryResponse);
  }

  /**
   * Get the documents from a query result.
   *
   * @param queryResult
   * @return
   */
  public static <T extends ServiceDocument> List<T> getQueryResultDocuments(
  ...
  /**
   * Gets the document links from a broadcast response.
   *
   * @param response
   * @return
   */
  public static Set<String> getBroadcastQueryResults(NodeGroupBroadcastResponse response) {
    if (!response.failures.isEmpty()) {
      throw new DcpRuntimeException("Failures detected in query task response: " + Utils.toJsonHtml(response));
    }
