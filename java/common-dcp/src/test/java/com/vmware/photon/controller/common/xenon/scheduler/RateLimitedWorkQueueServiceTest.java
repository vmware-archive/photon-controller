...
//          {startState, negativePendingPatchState},
  ...
  public class EndToEndTest {
    ...
    private final String WORK_QUEUE_NAME = "test-service-work-queue";

    private final String WORK_QUEUE_SELF_LINK = UriUtils.buildUriPath(RateLimitedWorkQueueFactoryService.SELF_LINK,
        WORK_QUEUE_NAME);

    private final Integer STRESS_TEST_ITERATIONS = 100;

    private final Integer STRESS_TEST_TASKS_PER_ITERATION = 200;
    ...
    public void setUpClass() throws Throwable {
      ...
      RateLimitedWorkQueueService.State startState = buildValidStartState();
      startState.documentSelfLink = WORK_QUEUE_NAME;
    ...
    private void testSingleSuccess() throws Throwable {
      ...
      startState.workQueueServiceLink = WORK_QUEUE_SELF_LINK;
    ...
    private void runStressIteration(int iteration) throws Throwable {
      ...
      for (int i = 0; i < STRESS_TEST_TASKS_PER_ITERATION; i++) {
        Operation op = basicHostEnvironment.sendPostAndWait(TestServiceWithWorkQueueFactory.SELF_LINK, startState);
        checkState(op.getStatusCode() == Operation.STATUS_CODE_OK);
      }
      ...
      basicHostEnvironment.waitForQuery(queryTask,
          (query) -> query.results.documentLinks.size() == (1 + iteration) * STRESS_TEST_TASKS_PER_ITERATION);

      logger.info("Finished stress iteration " + iteration);

      RateLimitedWorkQueueService.State queueState = basicHostEnvironment.getServiceState(WORK_QUEUE_SELF_LINK,
