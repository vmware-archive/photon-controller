...
public class ProvisionNetworkTaskService extends StatefulService {
  ...
  public static class ProvisionNetworkTaskState extends ServiceDocument {
    ...
    /**
     * The description of the network instance being realized.
     */
    public String networkDescriptionLink;

    /**
     * The network adapter to use to create the network.
     */
    public URI networkServiceReference;

    /**
     * The firewall adapter to use to create the firewall rules.
     */
    ...
    switch (patchState.taskInfo.stage) {
      case CREATED:
        ...
        logInfo("%s %s on %s started",
            currState.taskSubStage == SubStage.PROVISIONING_NETWORK ? "Network"
                : "Firewall",
  ...
  // deletes follow the inverse order;  remove the firewall before removing the network
  ...
  private void patchAdapter(ProvisionNetworkTaskState state) {
    ...
    URI adapter;
    if (state.taskSubStage == SubStage.PROVISIONING_NETWORK) {
      adapter = state.networkServiceReference;
    } else {
      adapter = state.firewallServiceReference;
    }

    sendRequest(Operation.createPatch(adapter)
        .setBody(req)
        .setCompletion((o, e) -> {
          if (e != null) {
