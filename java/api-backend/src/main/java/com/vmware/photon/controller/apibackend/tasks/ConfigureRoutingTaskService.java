...
public class ConfigureRoutingTaskService extends StatefulService {
  ...
  public void handleStart(Operation startOperation) {
    ...
    try {
      ...
      } else if (TaskState.TaskStage.CREATED == startState.taskState.stage) {
        TaskUtils.sendSelfPatch(this, buildPatch(TaskState.TaskStage.STARTED, TaskState.SubStage.CREATE_SWITCH_PORT));
      }
    } catch (Throwable t) {
    ...
    try {
      ...
      if (ControlFlags.isOperationProcessingDisabled(currentState.controlFlags)) {
        ...
        switch (currentState.taskState.subStage) {
          ...
          case CREATE_ROUTER_PORT:
            createRouterPort(currentState);
            break;

          case CONNECT_TIER0_ROUTER:
            connectTier0Router(currentState);
            break;

          default:
            throw new ConfigureRoutingException("Invalid task state " + currentState.taskState.stage);
        }
      }
    } catch (Throwable t) {
        ...
        new FutureCallback<LogicalPort>() {
          ...
          public void onSuccess(@Nullable LogicalPort result) {
            ConfigureRoutingTask patch = buildPatch(TaskState.TaskStage.STARTED,
                TaskState.SubStage.CREATE_ROUTER_PORT,
                null);
  ...
  private void createRouterPort(ConfigureRoutingTask currentState) throws IOException {
    NsxClientFactory factory = ((NsxClientFactoryProvider) getHost()).getNsxClientFactory();
    NsxClient nsxClient = factory.create(currentState.nsxManagerEndpoint, currentState.username, currentState.password);
        ...
        new FutureCallback<LogicalRouterDownLinkPort>() {
          ...
          public void onSuccess(@Nullable LogicalRouterDownLinkPort result) {
            ...
            if (currentState.routingType == RoutingType.ROUTED) {
              patch = buildPatch(TaskState.TaskStage.STARTED, TaskState.SubStage.CONNECT_TIER0_ROUTER, null);
            } else {
  ...
  private void connectTier0Router(ConfigureRoutingTask currentState) {
    // To be finished.
  ...
  private void validateStartState(ConfigureRoutingTask state) {
    checkState(state.taskState.stage == TaskState.TaskStage.CREATED,
        "Task stage must start from " + TaskState.TaskStage.CREATED);
  ...
  private void validatePatchState(ConfigureRoutingTask currentState, ConfigureRoutingTask patchState) {
    ...
    validateTaskStageProgression(currentState.taskState, patchState.taskState, currentState.routingType);
  }

  private void validateTaskStageProgression(TaskState startState, TaskState patchState, RoutingType routingType) {
    if (patchState.stage.ordinal() > TaskState.TaskStage.FINISHED.ordinal()) {
      return;
    }

    switch (startState.stage) {
      case CREATED:
        checkState(patchState.stage == TaskState.TaskStage.STARTED
            && patchState.subStage == TaskState.SubStage.CREATE_SWITCH_PORT);
        break;

      case STARTED:
        if (routingType == RoutingType.ROUTED) {
          checkState(startState.subStage == TaskState.SubStage.CONNECT_TIER0_ROUTER
              && patchState.stage.ordinal() > TaskState.TaskStage.STARTED.ordinal()
              || startState.subStage.ordinal() < TaskState.SubStage.CONNECT_TIER0_ROUTER.ordinal()
              && patchState.stage == TaskState.TaskStage.STARTED
              && startState.subStage.ordinal() + 1 == patchState.subStage.ordinal());
        } else {
          checkState(startState.subStage == TaskState.SubStage.CREATE_ROUTER_PORT
              && patchState.stage.ordinal() > TaskState.TaskStage.STARTED.ordinal()
              || startState.subStage.ordinal() < TaskState.SubStage.CREATE_ROUTER_PORT.ordinal()
              && patchState.stage == TaskState.TaskStage.STARTED
              && startState.subStage.ordinal() + 1 == patchState.subStage.ordinal());
        }
        break;
