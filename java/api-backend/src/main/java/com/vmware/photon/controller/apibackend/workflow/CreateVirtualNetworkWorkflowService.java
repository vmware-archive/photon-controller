...
public class CreateVirtualNetworkWorkflowService extends BaseWorkflowService {
  ...
  public static FactoryService createFactory() {
    return FactoryService.createIdempotent(CreateVirtualNetworkWorkflowService.class);
  }
  ...
  public void handleCreate(Operation createOperation) {
    ...
    try {
      ...
      validateState(state);
      processCreate(state, createOperation);
    } catch (Throwable t) {
  ...
  private void processCreate(CreateVirtualNetworkWorkflowDocument createState, Operation createOperation) {
    if (ControlFlags.isOperationProcessingDisabled(createState.controlFlags)) {
      ServiceUtils.logInfo(this, "Skipping create operation processing (disabled)");
      createOperation.complete();
      return;
    }

    createVirtualNetwork(createState, createOperation);
  }
