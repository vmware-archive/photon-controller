...
public class CreateVirtualNetworkWorkflowService extends BaseWorkflowService<CreateVirtualNetworkWorkflowDocument,
  ...
  /**
   * Get IPs for the virtual network.
   */
    ...
    ServiceHostUtils.getCloudStoreHelper(getHost())
        ...
        .setCompletion((op, ex) -> {
          ...
          try {
            ...
            patchState.taskServiceEntity.lowIpDynamic = IpHelper.longToDottedIp(subnet.lowIpDynamic);
            patchState.taskServiceEntity.highIpDynamic = IpHelper.longToDottedIp(subnet.highIpDynamic);
            patchState.taskServiceEntity.lowIpStatic = IpHelper.longToDottedIp(subnet.lowIpStatic);
            patchState.taskServiceEntity.highIpStatic = IpHelper.longToDottedIp(subnet.highIpStatic);
            if (subnet.reservedIpList != null && subnet.reservedIpList.isEmpty()) {
              for (Long ip : subnet.reservedIpList) {
                patchState.taskServiceEntity.reservedIpList.add(IpHelper.longToDottedIp(ip));
              }
            }
            progress(state, patchState);
          } catch (Throwable t) {
