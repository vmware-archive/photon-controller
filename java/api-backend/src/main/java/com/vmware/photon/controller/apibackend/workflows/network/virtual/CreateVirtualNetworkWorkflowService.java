...
import com.vmware.photon.controller.apibackend.servicedocuments.network.virtual.CreateLogicalRouterTask;
import com.vmware.photon.controller.apibackend.servicedocuments.network.virtual.CreateLogicalSwitchTask;
import com.vmware.photon.controller.apibackend.servicedocuments.network.virtual.CreateVirtualNetworkWorkflowDocument;
...
public class CreateVirtualNetworkWorkflowService extends BaseWorkflowService<CreateVirtualNetworkWorkflowDocument,
  ...
  private void setUpLogicalRouter(CreateVirtualNetworkWorkflowDocument state) {
    CreateLogicalPortsRoutingTask createLogicalPortsRoutingTask = new CreateLogicalPortsRoutingTask();
    createLogicalPortsRoutingTask.routingType = state.routingType;
    createLogicalPortsRoutingTask.nsxAddress = state.nsxAddress;
    createLogicalPortsRoutingTask.nsxUsername = state.nsxUsername;
    createLogicalPortsRoutingTask.nsxPassword = state.nsxPassword;
    createLogicalPortsRoutingTask.networkId = getVirtualNetworkId(state);
    createLogicalPortsRoutingTask.dhcpRelayServiceId = state.dhcpRelayServiceId;
    createLogicalPortsRoutingTask.logicalSwitchId = state.taskServiceEntity.logicalSwitchId;
    createLogicalPortsRoutingTask.logicalTier1RouterId = state.taskServiceEntity.logicalRouterId;
    createLogicalPortsRoutingTask.logicalTier0RouterId = state.tier0RouterId;
    createLogicalPortsRoutingTask.snatIp = state.snatIp;

    createLogicalPortsRoutingTask.logicalTier1RouterDownLinkPortIp = state.taskServiceEntity.reservedIpList
        .get(ReservedIpType.GATEWAY);
    createLogicalPortsRoutingTask.logicalTier1RouterDownLinkPortIpPrefixLen =
        32 - (int) (Math.log(state.size) / Math.log(2));

    TaskUtils.startTaskAsync(
        this,
        CreateLogicalPortsTaskService.FACTORY_LINK,
        createLogicalPortsRoutingTask,
        (st) -> TaskUtils.finalTaskStages.contains(st.taskState.stage),
        CreateLogicalPortsRoutingTask.class,
        state.subTaskPollIntervalInMilliseconds,
        new FutureCallback<CreateLogicalPortsRoutingTask>() {
          @Override
          public void onSuccess(CreateLogicalPortsRoutingTask result) {
            switch (result.taskState.stage) {
              case FINISHED:
                try {
                  CreateVirtualNetworkWorkflowDocument patchState = buildPatch(
