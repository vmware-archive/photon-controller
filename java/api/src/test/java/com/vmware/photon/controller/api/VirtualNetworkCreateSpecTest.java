...
public class VirtualNetworkCreateSpecTest {
  ...
  public class ValidationTest {
    ...
    public Object[][] getValidVirtualNetworkData() {
      return new Object[][] {
          {
              new VirtualNetworkCreateSpecBuilder().name("vn1").routingType(RoutingType.ROUTED).size(64).build()
          },
          {
              new VirtualNetworkCreateSpecBuilder().name("vn1").description("desc")
                  .routingType(RoutingType.ROUTED).size(64).reservedStaticIpSize(4).build()
          }
      };
    }
