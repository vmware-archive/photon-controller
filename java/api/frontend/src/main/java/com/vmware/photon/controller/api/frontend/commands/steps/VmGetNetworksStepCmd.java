...
public class VmGetNetworksStepCmd extends StepCommand {
  ...
  private final VmXenonBackend vmXenonBackend;
  ...
  public VmGetNetworksStepCmd(TaskCommand taskCommand,
                              ...
                              VmXenonBackend vmXenonBackend) {
    super(taskCommand, stepBackend, step);

    this.taskBackend = taskBackend;
    this.networkBackend = networkBackend;
    this.pollInterval = DEFAULT_POLL_INTERVAL;
    this.vmXenonBackend = vmXenonBackend;
  }
  ...
  protected void execute() throws ApiFeException, InterruptedException, RpcException {
    try {
      ...
      while(timeout > 0 && !areMACsPresent(response)) {
      ...
      if (needAnUpdate) {
        vmXenonBackend.updateState(vm, getNetworksMACAddresses(response));
      }
  ...
  private GetVmNetworkResponse getVmNetworksOpWithNoHostCachedInfo(VmEntity vm)
    ...
    } catch (com.vmware.photon.controller.api.frontend.exceptions.external.VmNotFoundException ex) {
