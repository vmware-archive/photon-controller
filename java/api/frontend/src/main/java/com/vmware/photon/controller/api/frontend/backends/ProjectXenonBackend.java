...
public class ProjectXenonBackend implements ProjectBackend {
  ...
  public ResourceList<Project> filter(String tenantId, Optional<String> name, Optional<Integer> pageSize) throws
      ExternalException {
    return filter(tenantId, name, pageSize, null);
  }
  ...
  public ResourceList<Project> filter(String tenantId, Optional<String> name, Optional<Integer> pageSize,
    ...
    ResourceList<ProjectService.State> projectDocuments = findByTenantIdAndName(tenantId, name, pageSize, tokenGroups);
  ...
  private ResourceList<ProjectService.State> findByTenantIdAndName(String tenantId,
    ...
    checkState(deploymentList.size() > 0, "Must have one or more deployments to list projects");
    Deployment deployment = deploymentList.get(0);
    boolean authEnabled = deployment.getAuth().getEnabled();

    // if auth enabled, filter project list to contain projects user has access to
    ...
    if (authEnabled){
      ...
      if (tokenGroups == null || tokenGroups.isEmpty()) {
        return PaginationUtils.xenonQueryResultToResourceList(ProjectService.State.class,
