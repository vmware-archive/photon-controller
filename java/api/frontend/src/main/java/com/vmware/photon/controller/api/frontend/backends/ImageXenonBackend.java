...
import com.vmware.photon.controller.api.frontend.exceptions.external.ImageNotFoundException.Type;
...
/**
 * Perform image related operations using Xenon based cloud store.
 */
@Singleton
public class ImageXenonBackend implements ImageBackend {

  private static final Logger logger = LoggerFactory.getLogger(ImageXenonBackend.class);
  ...
  public ImageEntity findById(String id) throws ExternalException {
    ...
    try {
      result = xenonClient.get(ImageServiceFactory.SELF_LINK + "/" + id);
  ...
  public boolean isImageSeedingDone(String imageId) throws ExternalException {
    try {
      ImageService.State state = xenonClient.get(ImageServiceFactory.SELF_LINK + "/" + imageId)
  ...
  private ServiceDocumentQueryResult findDocumentsByName(Optional<String> name, Optional<Integer> pageSize)
    ...
    return xenonClient.queryDocuments(ImageService.State.class, termsBuilder.build(), pageSize, true);
  }
  ...
  protected void updateImageCounts(String imageId) throws ExternalException {
    try {
      com.vmware.xenon.common.Operation result = xenonClient.postToBroadcastQueryService(buildDatastoreSetQuery());
      ...
      xenonClient.patch(ImageServiceFactory.SELF_LINK + "/" + imageId, patchState);
    } catch (DocumentNotFoundException e) {
      throw new ImageNotFoundException(Type.ID, imageId);
    }
  }
