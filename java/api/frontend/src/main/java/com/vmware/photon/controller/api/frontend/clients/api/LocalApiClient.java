...
public class LocalApiClient implements ApiClient {
  ...
  @Inject ClusterFeClient clusterFeClient;
  @Inject PaginationConfig paginationConfig;
  @Inject TaskFeClient taskFeClient;
  @Inject VmFeClient vmFeClient;

  public LocalApiClient() {
    this.tasksApi = new TasksLocalApi(taskFeClient);
    this.tenantsApi = new TenantsLocalApi();
    this.resourceTicketApi = new ResourceTicketLocalApi();
    this.projectApi = new ProjectLocalApi(vmFeClient);
    this.flavorApi = new FlavorLocalApi();
    this.disksApi = new DisksLocalApi();
    this.imagesApi = new ImagesLocalApi();
    this.vmApi = new VmLocalApi(vmFeClient);
    this.systemStatusApi = new SystemStatusLocalApi();
    this.clusterApi = new ClusterLocalApi(clusterFeClient, paginationConfig);
