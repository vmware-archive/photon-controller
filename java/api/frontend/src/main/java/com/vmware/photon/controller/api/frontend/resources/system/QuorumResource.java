...
@Path(SystemRoutes.GET_QUORUM)
public class QuorumResource {

    private final XenonRestClient xenonClient;
    private final static String NODE_GROUPS = "/core/node-groups/default";
    @Inject
    public QuorumResource(ApiFeXenonRestClient xenonClient){
    ...
    @ApiResponses(value = {@ApiResponse(code = 200 , message = "Returns the Quorum" )})
    public Response get(@Context Request request) throws InterruptedException, BadRequestException, TimeoutException, DocumentNotFoundException
    {
        ...
        for(Map.Entry<String, ?> entry: body.nodes.entrySet()) {
            jsonObject.put("Quorum", body.nodes.get((entry.getKey())).membershipQuorum);
        }
        return generateCustomResponse(Response.Status.OK, jsonObject);
    }

    @POST
    @ApiOperation(value = "Create a Photon Controller Quorum",
            notes = "This API helps create the Photon Controller Quorum.",
            response = System.class)
    @ApiResponses(value = {@ApiResponse(code = 200 , message = "Creates the Quorum" )})
    @Path(SystemRoutes.CREATE_QUORUM)
    public void post(@Context Request request, @PathParam("size") int size) throws InterruptedException, BadRequestException, TimeoutException, DocumentNotFoundException {
        updateQuorum(size);
    }
