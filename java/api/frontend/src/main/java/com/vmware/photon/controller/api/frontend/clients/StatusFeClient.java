...
public class StatusFeClient {
   ...
   * 1. Get addresses of all nodes in the defautl xenon node group.
  ...
  public SystemStatus getSystemStatus() throws InternalException {
    ...
    try {
      ...
      statusProviderFactories.put(Component.PHOTON_CONTROLLER,
          new XenonStatusProviderFactory(
              new StaticServerSet(servers.toArray(new InetSocketAddress[servers.size()])),
              this.executor,
              scheduledExecutorService,
              serviceHost));
    } catch (DocumentNotFoundException ex) {
      logger.error("Couldn't find nodegroup decument.", ex);
