...
public class HostCacheTest {
  ...
  public void testHostCache() throws Throwable {
    ...
    assertThat(hostCache.getNumberHosts(), equalTo(0));
    assertThat(hostCache.getNumberManagementHosts(), equalTo(0));
    assertThat(hostCache.getNumberDatastores(), equalTo(0));
  ...
  private void pollForHostCache(HostCache hostCache, int mgmtHostCount, int cloudHostCount, int datastoreCount)
    ...
    for (int i = 0; i < 20; i++) {
      if (hostCache.getNumberHosts() == totalHostCount && hostCache.getNumberManagementHosts() == mgmtHostCount
          && hostCache.getNumberDatastores() == datastoreCount) {
        break;
      }
      Thread.sleep(500);
    }

    assertThat(String.format("Expected %d hosts, found %d hosts", totalHostCount, hostCache.getNumberHosts()),
        hostCache.getNumberHosts(),
        equalTo(totalHostCount));
    assertThat(String.format("Expected %d management hosts, found %d management hosts",
        this.numManagementHosts, hostCache.getNumberManagementHosts()),
        hostCache.getNumberManagementHosts(), equalTo(mgmtHostCount));
    assertThat(String.format("Expected %d datastores, found %d datastores", this.numDatastores,
        hostCache.getNumberDatastores()),
        hostCache.getNumberDatastores(), equalTo(datastoreCount));
  }
