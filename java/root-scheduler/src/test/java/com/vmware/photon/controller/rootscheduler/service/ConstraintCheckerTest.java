...
public class ConstraintCheckerTest {
  ...
  public void testSingleConstraint(ConstraintChecker checker, Map<String, HostService.State> expectedHosts) {
    ...
    for (int i = 0; i < expectedHosts.size(); i++) {
      ...
      ResourceConstraint constraint = new ResourceConstraint(ResourceConstraintType.DATASTORE, Arrays.asList(dsName));
      constraints.add(constraint);
      Map<String, ServerAddress> candidates = checker.getCandidates(constraints, 2);
      ...
      constraints.add(constraint);
      candidates = checker.getCandidates(constraints, 10);
