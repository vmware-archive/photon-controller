...
public class FlatSchedulerServiceTest {
  ...
  @Mock
  private CloudStoreCache cloudStoreCache;

  @Mock
  private CloudStoreCacheFactory cloudStoreCacheFactory;
  ...
  public Object[][] createEmpty() {
    Map emptyMap = new HashMap<String, Map<String, CloudStoreCache.CachedDocument>>();
    emptyMap.put(HostServiceFactory.SELF_LINK, new HashMap<String, CloudStoreCache.CachedDocument>());
    emptyMap.put(DatastoreServiceFactory.SELF_LINK, new HashMap<String, CloudStoreCache.CachedDocument>());
    doReturn(emptyMap).when(cloudStoreCache).getCurrentData();
    doReturn(cloudStoreCache).when(cloudStoreCacheFactory).create(any());
    RootScheduler.Iface inMemory = new FlatSchedulerService(config, hostClientFactory, cloudStoreCacheFactory);
    return new Object[][]{
        {inMemory},
    };
  }
