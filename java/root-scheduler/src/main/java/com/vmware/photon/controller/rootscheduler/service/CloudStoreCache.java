...
public class CloudStoreCache {
  ...
  public final Map<String, Map<String, CachedDocument>> currentData;
  ...
  private void processQuery(ExpandQueryResponse queryResponse, String path) {
    ...
    for (String uri : queryResponse.documents.keySet()) {
      ...
      if (currentDocument == null) {
        ...
        if (currentDocument.version < newDocument.version) {
          // There is a newer version of this path, update
          // the current view
          pathResources.put(id, newDocument);
        } else if (currentDocument.version > newDocument.version) {
          logger.error("Ignoring event, resource {}/{} version decreased from {} to {}",
              path, id,
              currentDocument.version, newDocument.version);
        }
      }
    }
  ...
  public void refresh() {
    ...
    for (String prefixPath : currentData.keySet()) {
      try {
        ...
        logger.info("Took {} seconds to get {}, query time {} us", timer.elapsed(TimeUnit.SECONDS),
  ...
  public Map<String, Map<String, CachedDocument>> getCurrentData() {
    return currentData;
  }
  ...
  public class CachedDocument {
    ...
    public final ServiceDocument document;
