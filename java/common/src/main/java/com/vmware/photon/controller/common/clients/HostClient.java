...
import com.vmware.photon.controller.host.gen.CreateDirectoryRequest;
import com.vmware.photon.controller.host.gen.CreateDirectoryResponse;
...
public class HostClient {
  ...
  private static final long CREATE_DIRECTORY_TIMEOUT_MS = 60000;
  ...
  /**
   * This method performs a synchronous Thrift call to create a directory.
   *
   * @param directoryPath Supplies the path of the directory to be created.
   * @param dataStore     Supplies the data store on which the directory exists.
   * @return On success, the return value is the CreateDirectoryResponse object
   * generated by the Thrift call.
   * @throws InterruptedException
   * @throws RpcException
   */
  public CreateDirectoryResponse createDirectory(String directoryPath, String dataStore)
          throws InterruptedException, RpcException {
    SyncHandler<CreateDirectoryResponse, Host.AsyncClient.create_directory_call> syncHandler = new SyncHandler<>();
    createDirectory(directoryPath, dataStore, syncHandler);
    syncHandler.await();
    return ResponseValidator.checkCreateDirectoryResponse(syncHandler.getResponse());
  }

  /**
   * This method performs a asynchronous Thrift call to create a directory. On
   * completion, the specified handler is invoked.
   *
   * @param directoryPath Supplies the path of the directory to be created.
   * @param dataStore     Supplies the data store on which the directory exists.
   ...
   */
  @RpcMethod
  public void createDirectory(String directoryPath, String dataStore,
                              AsyncMethodCallback<Host.AsyncClient.create_directory_call> handler)
          throws RpcException {
    ensureClient();
    CreateDirectoryRequest createDirectoryRequest = new CreateDirectoryRequest(dataStore, directoryPath);
    clientProxy.setTimeout(CREATE_DIRECTORY_TIMEOUT_MS);
    logger.info("create_directory target {}, request {]", getTarget(), createDirectoryRequest);

    try {
      clientProxy.create_directory(createDirectoryRequest, handler);
    } catch (TException e) {
  ...
  public void getAgentStatus(AsyncMethodCallback<Host.AsyncClient.get_agent_status_call> handler)
    ...
    logger.info("get_agent_status target {}, request {}", getTarget());
  ...
  public AgentStatusResponse getAgentStatus()
      throws InterruptedException, RpcException {
    SyncHandler<AgentStatusResponse, Host.AsyncClient.get_agent_status_call> syncHandler = new SyncHandler<>();
  ...
  public static class ResponseValidator {
    ...
    private static CreateDirectoryResponse checkCreateDirectoryResponse(CreateDirectoryResponse createDirectoryResponse)
            throws RpcException {
      logger.info("Checking {}", createDirectoryResponse);
      switch (createDirectoryResponse.getResult()) {
        case OK:
          break;
        case DIRECTORY_ALREADY_EXISTS:
          throw new DirectoryAlreadyExistsException(createDirectoryResponse.getError());
        case DATASTORE_NOT_FOUND:
          throw new DatastoreNotFoundException(createDirectoryResponse.getError());
        case SYSTEM_ERROR:
          throw new SystemErrorException(createDirectoryResponse.getError());
        default:
          throw new RpcException(String.format("Unknown result: %s", createDirectoryResponse.getError()));
      }
