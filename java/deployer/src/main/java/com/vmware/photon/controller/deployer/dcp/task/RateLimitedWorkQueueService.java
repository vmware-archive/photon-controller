 ...
 * items from CREATED to STARTED state as they arrive up to a user-specified concurrency limit.
 *
 * ** The concurrency limit is per-node, not global, so this service will not behave as expected for services which are
 * REPLICATED and OWNER_SELECTED. **
 */
public class RateLimitedWorkQueueService extends StatefulService {
  ...
  private void registerWorkItemQuery(Operation postOp, State startState) {
    ...
    //
    // N.B. It is currently impossible to specify that a QueryTask document should not expire. Until this capability is
    // supported in Xenon, this workaround is necessary.
    //
