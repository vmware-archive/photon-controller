...
import com.vmware.photon.controller.common.xenon.RateLimitedWorkQueueFactoryService;
import com.vmware.photon.controller.common.xenon.RateLimitedWorkQueueService;
...
public class DeployerXenonServiceHost
  ...
  private void startCoreServices() {
        ...
        (o, e) -> {
          ...
          for (String link : TASK_SCHEDULERS.keySet()) {
            try {
              ...
              // to start
              logger.warn("Could not start task scheduler service [%s]: %s", link, Utils.toString(t));
            }
          }
    ...
    registerForServiceAvailability(
        (o, e) -> {
          ...
          for (String kind : WORK_QUEUES.keySet()) {
            try {
              startWorkQueueService(kind, WORK_QUEUES.get(kind));
            } catch (Throwable t) {
              logger.warn("Could not start work queue service [%s]: %s", kind, Utils.toString(t));
            }
          }
  ...
  private void startWorkQueueService(String workItemKind, Integer concurrencyLimit) {

    RateLimitedWorkQueueService.State startState = new RateLimitedWorkQueueService.State();
    startState.workItemKind = workItemKind;
