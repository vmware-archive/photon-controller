...
public class BuildContainersConfigurationWorkflowService extends StatefulService {
  ...
  public static class State extends ServiceDocument {
    ...
    /**
     * This value represents the optional document self-link of the VM service document whose
     * containers should be processed by the current task (e.g. when this task is called from the
     * "add management host" workflow). If this value is not specified, then all containers will be
     * processed.
     */
    @Immutable
  ...
  private void processBuildRuntimeConfigurationSubStage(State currentState) {
    ...
    if (currentState.vmServiceLink != null) {
      queryBuilder.addFieldClause(ContainerService.State.FIELD_NAME_VM_SERVICE_LINK, currentState.vmServiceLink);
    }
        ...
        .setCompletion(
            (o, e) -> {
              ...
              try {
                ...
                getZookeeperContainers(currentState, templateServiceLinks.iterator().next());
              } catch (Throwable t) {
                failTask(t);
              }
            }));
  ...
  private void getZookeeperContainers(State currentState, String templateServiceLink) {
    ...
    OperationJoin
        .create(containerDocuments.values().stream().map(
            (containerDocument) -> {
              ...
              patchState.dynamicParameters.put("ZOOKEEPER_INSTANCES", zookeeperInstances);
              return HostUtils.getCloudStoreHelper(this).createPatch(containerState.documentSelfLink)
                  .setBody(patchState);
            }))
        .setCompletion(
            (ops, exs) -> {
