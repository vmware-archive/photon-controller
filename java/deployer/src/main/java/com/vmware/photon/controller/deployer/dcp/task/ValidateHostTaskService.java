...
import com.vmware.photon.controller.deployer.service.exceptions.ManagementVmIpAlreadyAssignToHost;
...
public class ValidateHostTaskService extends StatefulService {
  ...
  private void validateHost(State state) throws Throwable {
    ...
    System.out.println(cloudStoreUri.toString());
    ...
    OperationSequence.create(uniqueHostIpQuery, allMgmtHostsQuery)
      .setCompletion((os, ts) -> {
        ...
        } catch (ManagementVmIpAlreadyAssignToHost t) {
  ...
  private void validateMgmtVmIpNotAssigned(State state, Operation allMgmtHostsQuery, Map<Long, Operation> os)
      throws ManagementVmIpAlreadyAssignToHost {
    ...
    if (!mgmtHostsWithSameMgmtVmIp.isEmpty()) {
      throw new ManagementVmIpAlreadyAssignToHost(
