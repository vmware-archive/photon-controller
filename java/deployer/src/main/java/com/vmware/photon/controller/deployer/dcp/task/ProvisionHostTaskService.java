...
public class ProvisionHostTaskService extends StatefulService {
  ...
  /**
   * The number of times the task tries to check if the Host Service
   * has been updated with configuration data. The interval between
   * each retry is 5secs, so overall we will wait for 1 minute approx.
   */
  ...
  private void sendStageProgressPatch(State currentState, State patchState) {
    ServiceUtils.logTrace(this, "Sending self-patch to stage %s : %s",
