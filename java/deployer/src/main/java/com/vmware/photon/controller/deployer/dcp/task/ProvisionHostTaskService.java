...
public class ProvisionHostTaskService extends StatefulService {
  ...
  public static class State extends ServiceDocument {
    ...
    /**
     * This value represents the maximum number of attempts which should be made to call the
     * provision interface on the newly-deployed agent before declaring failure.
     */
    @DefaultInteger(value = 60)
    @Positive
    @Immutable
    public Integer maximumProvisionCount;

    /**
     * This value represents the interval, in milliseconds, to wait between attempts to provision
     * the agent.
     */
    @DefaultInteger(value = 5000)
    @Positive
    @Immutable
    public Integer provisionPollDelay;

    /**
     * This value represents the number of attempts which have been made by the current task to
     * provision the agent.
     */
    @DefaultInteger(value = 0)
    public Integer provisionCount;

    /**
     * This value represents the maximum number of attempts which should be made to call the
     * getHostConfig interface on the newly-provisioned agent before declaring failure.
     */
    @DefaultInteger(value = 60)
    @Positive
    @Immutable
    public Integer maximumAgentStatusCount;

    /**
     * This value represents the interval, in milliseconds, to wait between attempts to get the
     * host configuration from the agent.
     */
    @DefaultInteger(value = 5000)
    @Positive
    @Immutable
    ...
    /**
     * This value represents the number of attempts which have been made by the current task to
     * get the host configuration from the agent.
     */
    @DefaultInteger(value = 0)
    public Integer agentStatusCount;
  }
  ...
  private void validatePatchState(State currentState, State patchState) {
    ...
    if (patchState.provisionCount != null && currentState.provisionCount != null) {
      checkState(patchState.provisionCount >= currentState.provisionCount);
    }

    if (patchState.agentStatusCount != null && currentState.agentStatusCount != null) {
      checkState(patchState.agentStatusCount >= currentState.agentStatusCount);
    }
  }
  ...
  private void validateTaskStageProgression(TaskState currentState, TaskState patchState) {
    ...
    if (currentState.stage == TaskState.TaskStage.STARTED && patchState.stage == TaskState.TaskStage.STARTED) {
  ...
  private void processStartedStage(State currentState,
    ...
    switch (currentState.taskState.subStage) {
      case INSTALL_AGENT:
        ...
        break;
      case PROVISION_AGENT:
        processProvisionAgentStage(currentState, deploymentState, hostState);
        break;
      case WAIT_FOR_AGENT:
  ...
  private void processProvisionAgentStage(State currentState,
                                          DeploymentService.State deploymentState,
                                          HostService.State hostState) {
    List<String> dataStores = null;
    if (hostState.metadata != null && hostState.metadata.containsKey(
    ...
    boolean isManagementOnly = (hostState.usageTags != null
    ...
    try {
      ...
      hostClient.provision(
          ...
          hostState.agentPort,
          new ArrayList<>(deploymentState.chairmanServerList),
          ...
          new AsyncMethodCallback<Host.AsyncClient.provision_call>() {
            ...
            public void onComplete(Host.AsyncClient.provision_call provisionCall) {
              try {
                ...
                retryProvisioningOrFail(currentState, t);
              }
            }

            @Override
            public void onError(Exception e) {
              retryProvisioningOrFail(currentState, e);
            }
          });
    } catch (Throwable t) {
      retryProvisioningOrFail(currentState, t);
  ...
  private void retryProvisioningOrFail(State currentState, Throwable t) {
    if (currentState.provisionCount + 1 >= currentState.maximumProvisionCount) {
      ServiceUtils.logSevere(this, t);
      State patchState = buildPatch(TaskState.TaskStage.FAILED, null, new IllegalStateException(
          "Provisioning the agent on host " + this.hostState.hostAddress + " failed after " +
              Integer.toString(currentState.maximumProvisionCount) + " retries"));
      patchState.provisionCount = currentState.provisionCount + 1;
      TaskUtils.sendSelfPatch(this, patchState);
    } else {
      ServiceUtils.logTrace(this, t);
      State patchState = buildPatch(TaskState.TaskStage.STARTED, TaskState.SubStage.PROVISION_AGENT, null);
      patchState.provisionCount = currentState.provisionCount + 1;
      getHost().schedule(() -> TaskUtils.sendSelfPatch(this, patchState), currentState.provisionPollDelay,
      ...
      hostClient.getAgentStatus(new AsyncMethodCallback<Host.AsyncClient.get_agent_status_call>() {
        ...
        public void onComplete(Host.AsyncClient.get_agent_status_call getAgentStatusCall) {
          try {
            ...
            logInvalidAgentConfigAndFail(hostState, e);
          } catch (Throwable t) {
  ...
  private void logInvalidAgentConfigAndFail(HostService.State hostState, InvalidAgentConfigurationException e) {
    ServiceUtils.logSevere(this, e);
    TaskUtils.sendSelfPatch(this, buildPatch(TaskState.TaskStage.FAILED, null, new IllegalStateException(
        "The agent configuration supplied to host " + hostState.hostAddress + " was invalid: " + e.getMessage())));
  }

  private void retryGetAgentStatusOrFail(State currentState, Throwable t) {
    if (currentState.agentStatusCount + 1 >= currentState.maximumAgentStatusCount) {
      ServiceUtils.logSevere(this, t);
      State patchState = buildPatch(TaskState.TaskStage.FAILED, null, new IllegalStateException(
          "The agent on host " + this.hostState.hostAddress + " failed to become ready after provisioning after " +
              Integer.toString(currentState.maximumAgentStatusCount) + " retries"));
      patchState.agentStatusCount = currentState.agentStatusCount + 1;
      TaskUtils.sendSelfPatch(this, patchState);
    } else {
      ServiceUtils.logTrace(this, t);
      State patchState = buildPatch(TaskState.TaskStage.STARTED, TaskState.SubStage.WAIT_FOR_AGENT, null);
      patchState.agentStatusCount = currentState.agentStatusCount + 1;
      getHost().schedule(() -> TaskUtils.sendSelfPatch(this, patchState), currentState.agentStatusPollDelay,
          TimeUnit.MILLISECONDS);
    }
  }
