...
public class InitializeDeploymentMigrationWorkflowService extends StatefulService {
  ...
  private void pauseDestinationSystem(State currentState) throws Throwable {
    final State currentState1 = currentState;
    final FutureCallback<ResourceList<Deployment>> querySourceDeploymentCallback = new
        TaskFailingCallback<ResourceList<Deployment>>(this) {
          @Override
          public void onSuccess(@Nullable final ResourceList<Deployment> result) {
            try {
              ...
              pauseDestinationSystem(sourceDeploymentId, currentState.destinationDeploymentId, currentState1);
    ...
    getDeployment(currentState1, currentState1.sourceLoadBalancerAddress, querySourceDeploymentCallback);
  }

  private void pauseDestinationSystem(final String sourceDeploymentId, final String destinationDeploymentId,
      final State currentState) throws Throwable {
  ...
  private OperationJoin createStartMigrationOperations(State currentState) {
    ...
    List<Operation> cloudStoreOperations = HostUtils.getDeployerContext(this).getCloudStoreFactoryLinkMapEntries()
        ...
        .map(entry -> {
          ...
          InetSocketAddress local = ServiceUtils.selectRandomItem(cloudStoreSourceServers);
    ...
    List<Operation> deployerOperations = HostUtils.getDeployerContext(this).getDeployerFactoryLinkMapEntries().stream()
        .map(entry -> {
          String destinationFactoryLink = entry.getValue();
          String sourceFactoryLink = entry.getKey();
          InetSocketAddress local = ServiceUtils.selectRandomItem(deployerSourceServers);
          InetSocketAddress remote = ServiceUtils.selectRandomItem(deployerDestinationServers);
          CopyStateTriggerTaskService.State startState = new CopyStateTriggerTaskService.State();
          startState.sourceIp = local.getAddress().getHostAddress();
          startState.sourcePort = local.getPort() + 1;
          startState.destinationIp = remote.getAddress().getHostAddress();
  ...
  private void waitUntilCopyStateTasksFinished(CompletionHandler handler, State currentState) {
    ...
    generateQueryCopyStateTaskTriggerQuery()
      .setCompletion((op, t) -> {
        if (t != null) {
          handler.handle(op, t);
          return;
