...
function get_rest_response() {
  ...
  if [[ ! -n "$payload" ]]; then
    local curl_cmd="curl -sS -k -u $NETWORK_MANAGER_USERNAME:$NETWORK_MANAGER_PASSWORD https://$NETWORK_MANAGER_ADDRESS/$api"
  else
    local curl_cmd="curl -sS -H \"Content-Type: application/json\" -k -u $NETWORK_MANAGER_USERNAME:$NETWORK_MANAGER_PASSWORD -X POST -d $payload https://$NETWORK_MANAGER_ADDRESS/$api"
  fi
  ...
  local ip_pool_json="'{ \
    ...
    \"subnets\": [ \
      { \
          ...
          \"allocation_ranges\": [ \
              { \
                  \"start\": \"$NETWORK_EDGE_IP_POOL_START\", \
                  \"end\": \"$NETWORK_EDGE_IP_POOL_END\"
              } \
          ], \
          \"cidr\": \"$NETWORK_EDGE_IP_POOL_CIDR\"
      }
    ]
  }'"
...
function edit_router_advertisement() {
  ...
  cmd="curl -sS -H \"Content-Type: application/json\" -k -u $NETWORK_MANAGER_USERNAME:$NETWORK_MANAGER_PASSWORD -X PUT -d $edit_router_advertisement_json https://$NETWORK_MANAGER_ADDRESS/api/v1/logical-routers/${1}/routing/advertisement"
...
check_environment_variable "$NETWORK_EDGE_IP_POOL_CIDR" "NETWORK_EDGE_IP_POOL_CIDR"
check_environment_variable "$NETWORK_EDGE_IP_POOL_START" "NETWORK_EDGE_IP_POOL_START"
check_environment_variable "$NETWORK_EDGE_IP_POOL_END" "NETWORK_EDGE_IP_POOL_END"
check_environment_variable "$NETWORK_T0_SUBNET_IP_ADDRESS" "NETWORK_T0_SUBNET_IP_ADDRESS"
check_environment_variable "$NETWORK_T0_SUBNET_LENGTH" "NETWORK_T0_SUBNET_LENGTH"
...
check_environment_variable "$NETWORK_DHCP_SUBNET_LENGTH" "NETWORK_DHCP_SUBNET_LENGTH"
...
response=$(create_uplink_router_port "to-vlan-logical-switch" $t0_router_id $logical_port_id $NETWORK_T0_SUBNET_IP_ADDRESS $NETWORK_T0_SUBNET_LENGTH)
check_for_error "$response"
...
response=$(create_downlink_router_port "to-dhcp-switch-port" $dhcp_router_id $dhcp_logical_port_id $NETWORK_DHCP_SUBNET_IP_ADDRESS $NETWORK_DHCP_SUBNET_LENGTH)
check_for_error "$response"
...
edit_router_advertisement $dhcp_router_id

echo "NSX configuration completed successfully"

echo ""
echo "vlan transport id:" $vlan_transport_id
echo "overlay transport id:" $overlay_transport_id
echo "uplink id:" $uplink_profile_id
echo "ip_pool id:" $ip_pool_id
echo "edge transport node id:" $transport_node_id
echo "edge cluster id:" $edge_cluster_id
echo "T0 router id:" $t0_router_id
echo "T0 vlan logical switch id:" $logical_switch_id
echo "T0 vlan logical port id:" $logical_port_id
echo "T0 vlan router port id:" $t0_rourter_port_id
echo "dhcp router id:" $dhcp_router_id
echo "t0 to dhcp router port id:" $t0_link_router_port_for_dhcp_router_id
echo "dhcp to t0 router port id:" $dhch_router_port_link_t0_id
echo "dhcp logical switch id:" $dhcp_logical_switch_id
echo "dhcp logical port id:" $dhcp_logical_port_id
echo "dhcp router port id:" $dhcp_rourter_port_id
