...
In addition to the usual `vagrant up`, the Photon Controller service containers to be built into it. We use Gradle for building these containers and managing the lifecycle of the devbox. From `<code>/java`, run:

	./gradlew :devbox:buildAll :devbox:startAll
...
The Gradle build system under `<code>/java` provides tasks for managing the devbox.

	# Equivalent to vagrant up
	./gradlew :devbox:up

	# Equivalent to vagrant destroy
	./gradlew :devbox:destroy

	# Equivalent to vagrant halt
	./gradlew :devbox:halt

	# Equivalent to vagrant reload
	./gradlew :devbox:reload

	# Equivalent to vagrant status
	./gradlew :devbox:status

	# Equivalent to vagrant destroy followed by vagrant up
	...
	# Builds container for serviceName, e.g. deployer, management-api, etc
	./gradlew :serviceName:container

Task definitions can be found in their respective Gradle build files. The usual Gradle project layout is used. E.g., devbox tasks are under `<code>/java/devbox/build.gradle`, deployer tasks are under `<code>/java/deployer/build.gradle`, etc.
