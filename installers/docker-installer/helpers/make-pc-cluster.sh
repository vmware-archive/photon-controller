...
if [ "$1" == "multi" ]; then
  ./helpers/run-pc-container.sh $PHOTON_PEER_1 $PHOTON_PEER_0 $PHOTON_PEER_2 $LIGHTWAVE_PARTNER_0 $LIGHTWAVE_LOAD_BALANCER_IP ${PC_NAME_POSTFIX}1
  ./helpers/run-pc-container.sh $PHOTON_PEER_2 $PHOTON_PEER_0 $PHOTON_PEER_1 $LIGHTWAVE_PARTNER_0 $LIGHTWAVE_LOAD_BALANCER_IP ${PC_NAME_POSTFIX}2
fi
