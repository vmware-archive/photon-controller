...
MGMT_UI_LOGIN_URL=$(docker exec -it photon-controller-1 curl --key /etc/keys/machine.privkey --cert /etc/keys/machine.crt --capath /etc/ssl/certs https://192.168.114.11:19000/photon/cloudstore/deployments/default | json_pp | grep oAuthMgmtUiLoginEndpoint | cut -f4 -d '"')
MGMT_UI_LOGOUT_URL=$(docker exec -it photon-controller-1 curl --key /etc/keys/machine.privkey --cert /etc/keys/machine.crt --capath /etc/ssl/certs https://192.168.114.11:19000/photon/cloudstore/deployments/default | json_pp | grep oAuthMgmtUiLogoutEndpoint | cut -f4 -d '"')
